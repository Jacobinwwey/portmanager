...
| --- | --- | --- |
| `hosts` | V1 managed-host resource | Controller and CLI implemented. `apps/controller/src/controller-server.ts` exposes `/hosts`, `/hosts/{hostId}`, `/hosts/{hostId}/probe`, and `/hosts/{hostId}/bootstrap`; `crates/portmanager-cli/src/main.rs` now mirrors list/get/create/probe/bootstrap; `apps/web/src/main.ts` still has no real `Hosts` page. |
| `bridge-rules` | CRUD plus verification and drift handling | Controller and CLI implemented. Controller exposes list/detail/create/update/delete plus `/bridge-rules/{ruleId}/drift-check`; CLI now mirrors list/get/create/update/delete; web parity is still missing. |
| `exposure-policies` | Host-level policy read/write surface | Controller and CLI implemented. `GET/PUT /exposure-policies/{hostId}` are real in controller, and CLI now mirrors get/set; web parity is still missing. |
...
| 表面 | 契约预期 | 当前已验证状态 |
| --- | --- | --- |
| `hosts` | V1 受管主机资源 | controller 与 CLI 已实现。`apps/controller/src/controller-server.ts` 已暴露 `/hosts`、`/hosts/{hostId}`、`/hosts/{hostId}/probe`、`/hosts/{hostId}/bootstrap`；`crates/portmanager-cli/src/main.rs` 也已补齐 list/get/create/probe/bootstrap；`apps/web/src/main.ts` 仍没有真实 `Hosts` 页面。 |
| `bridge-rules` | CRUD 加验证与漂移处理 | controller 与 CLI 已实现。controller 已暴露 list/detail/create/update/delete 以及 `/bridge-rules/{ruleId}/drift-check`；CLI 也已补齐 list/get/create/update/delete；web 一致性仍未补齐。 |
| `exposure-policies` | 主机级 policy 读写面 | controller 与 CLI 已实现。`GET/PUT /exposure-policies/{hostId}` 已真实存在，CLI 也已补齐 get/set；web 一致性仍未补齐。 |
