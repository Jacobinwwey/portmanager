...
- The promotion-review helper composes the sync and review-digest steps for the default developer-review flow, writes `.portmanager/reports/milestone-wording-review.md` for human milestone-language review, publishes `Public claim class` plus `Required next action`, and only refreshes the tracked public artifact when `--refresh-published-artifact` is passed explicitly.
...
- 这条 promotion-review helper 会把 sync 与 review-digest 串成默认开发者复核链路，还会额外写出 `.portmanager/reports/milestone-wording-review.md` 供人工里程碑文案复核使用，明确给出 `Public claim class` 与 `Required next action`，并且只有在显式传入 `--refresh-published-artifact` 时才会刷新被跟踪公开 artifact。
