...
Updated: 2026-04-21
Version: v0.5.21-confidence-refresh-maintenance
...
- `Next lane`: Milestone 2 promotion-ready publication refresh and maintenance on the same live slice: keep `pnpm milestone:review:promotion-ready -- --limit 20` as the default completed-mainline review entrypoint, use `pnpm milestone:fetch:review-pack` when the current CI run is the first question, treat `.portmanager/reports/current-ci-review-pack/`, `.portmanager/reports/milestone-wording-review.md`, `Public claim class`, `Source surface status`, the verification report, and the public development-progress page as one review bundle, refresh the tracked public snapshot only when the helper reports `promotion-ready-refresh-required` and review agrees, and keep qualified history green while human milestone-language review stays deliberate.
...
- `下一主线`：继续在同一条 live 切片上推进 Milestone 2 的 promotion-ready 发布刷新与维护：把 `pnpm milestone:review:promotion-ready -- --limit 20` 作为 completed-mainline 之后的默认复核入口；如果第一问题是当前 CI run，就先执行 `pnpm milestone:fetch:review-pack` 并读取 `.portmanager/reports/current-ci-review-pack/`；随后把 `.portmanager/reports/milestone-wording-review.md`、`Public claim class`、`Source surface status`、验证报告与公开 development-progress 页面视为同一份复核包，只在 helper 报告 `promotion-ready-refresh-required` 且人工复核同意时通过同一条 helper 加上 `--refresh-published-artifact` 推进公开快照，并继续把 qualified history 保持为绿。
