...
- Real today from Unit 6 and Unit 7: `pnpm milestone:verify:confidence` now composes the standing `pnpm acceptance:verify` gate with the remote-backup replay proof, while `.github/workflows/mainline-acceptance.yml` now runs that heavier confidence routine on `push main` and `workflow_dispatch` without redefining the lighter PR gate.
- Real today from the next confidence-maintenance slice: each `pnpm milestone:verify:confidence` run now writes `.portmanager/reports/milestone-confidence-report.json`, and the mainline confidence job now uploads that report as a CI artifact so repeat-green evidence stops living only in raw logs.
- Deep compare against the `2026-04-16` requirements and reconciliation plan now shows that the old Unit 1 through Unit 7 proof-orchestration gap is closed in code; the remaining technical gap is confidence accumulation, because the canonical routine and report artifact now exist but still need repeated green history before Milestone 2 wording can tighten again.
- New direction docs now lock that next lane in `docs/brainstorms/2026-04-17-portmanager-m2-confidence-routine-requirements.md` and `docs/plans/2026-04-17-portmanager-m2-confidence-routine-plan.md`.
- Active execution lane after the remote-backup-replay slice is now Milestone 2 confidence-routine maintenance: keep `pnpm milestone:verify:confidence` green on the same accepted live slice long enough for milestone wording to earn further simplification.
...
- `Unit 7`: complete. `.github/workflows/mainline-acceptance.yml` now keeps PR validation on `pnpm acceptance:verify` and runs `pnpm milestone:verify:confidence` on `push main` and `workflow_dispatch`.
- `Confidence report slice`: complete. `pnpm milestone:verify:confidence` now writes `.portmanager/reports/milestone-confidence-report.json`, and CI uploads the same report as `milestone-confidence-report-*` for developers to inspect.
- `Next lane`: Milestone 2 confidence-routine maintenance on the same live slice: keep the new canonical routine green long enough to earn simpler milestone wording.
...
6. `docs/specs/portmanager-v1-product-spec.md`
7. `docs/architecture/portmanager-v1-architecture.md`
8. `docs/design/portmanager-overview-design-baseline.md`
9. `docs/design/portmanager-docs-site-design-baseline.md`
...
- 当前也已补上 Unit 6 与 Unit 7：`pnpm milestone:verify:confidence` 现在已经把既有 `pnpm acceptance:verify` gate 与 remote-backup replay proof 收敛成一条规范 routine；`.github/workflows/mainline-acceptance.yml` 也已在 `push main` 与 `workflow_dispatch` 上运行这条更重的 routine，同时保留更轻的 PR gate。
- 当前也已补上下一段 confidence-maintenance 切片：每次 `pnpm milestone:verify:confidence` 运行现在都会写出 `.portmanager/reports/milestone-confidence-report.json`，主线 confidence job 也会把同一份报告上传成 CI artifact，让持续转绿历史不再只停留在原始日志里。
- 深度对比 `2026-04-16` 的需求文档与 reconciliation plan 之后，现在已经可以确认：旧的 Unit 1 到 Unit 7 证明编排缺口都已在代码中闭环；剩余技术缺口已经收窄为 confidence 积累，因为规范 routine 与报告 artifact 已存在，但仍需要持续为绿的历史，Milestone 2 文案才有资格继续收窄。
- 新的方向文档已经把下一主线锁定在 `docs/brainstorms/2026-04-17-portmanager-m2-confidence-routine-requirements.md` 与 `docs/plans/2026-04-17-portmanager-m2-confidence-routine-plan.md`。
- remote-backup-replay 切片之后，主动执行主线继续收窄为 Milestone 2 的 confidence-routine 维护：继续把 `pnpm milestone:verify:confidence` 在同一条 live 切片上维持为绿，再用持续为绿的历史去换取后续里程碑文案收窄。
...
- `Unit 7`：已完成。`.github/workflows/mainline-acceptance.yml` 现在会继续用 `pnpm acceptance:verify` 作为 PR gate，并在 `push main` 与 `workflow_dispatch` 上运行 `pnpm milestone:verify:confidence`。
- `Confidence report 切片`：已完成。`pnpm milestone:verify:confidence` 现在会写出 `.portmanager/reports/milestone-confidence-report.json`，CI 也会把同一份报告上传成 `milestone-confidence-report-*` artifact 供开发者核对。
- `下一主线`：继续在同一条 live 切片上推进 Milestone 2 的 confidence-routine 维护：先把新 routine 持续保持为绿，再把当前可信度从“新结论”变成“常态”。
