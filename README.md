...
- Latest remote proof on `2026-04-17`: GitHub Actions runs `24565361391` (`mainline-acceptance`) and `24565361388` (`docs-pages`) both completed successfully on `main` for commit `63a1257`, so Unit 0 is now achieved as standing branch discipline.
...
- Active execution lane is now Unit 1 controller surface parity; Unit 0 remains mandatory, but no longer defines the main development objective.
...
- `Unit 1`: make controller `hosts`, `bridge-rules`, and `exposure-policies` real through the shared store and runner, without regressing backup, rollback, diagnostics, drift, or event evidence.
- `Unit 2`: mirror those resources into Rust CLI read and write flows so CLI becomes a truthful peer to controller contracts.
...
- 最新远端证明发生在 `2026-04-17`：GitHub Actions runs `24565361391`（`mainline-acceptance`）与 `24565361388`（`docs-pages`）已经在 `main` 的 commit `63a1257` 上同时通过，因此 Unit 0 现在已经从“待建立纪律”转为“持续保持的分支纪律”。
...
- 当前主动执行主线已经转到 Unit 1 controller 表面一致性；Unit 0 仍然必须保持，但不再是当前开发目标本身。
...
### 当前执行顺序
- `Unit 1`：先把 controller 的 `hosts`、`bridge-rules`、`exposure-policies` 做成真实表面，并继续共用现有 store / runner 与备份、回滚、诊断、drift、事件证据链。
