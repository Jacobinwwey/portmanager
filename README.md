...
- New direction docs now extend that chain through the full Milestone 2 confidence series and into Milestone 3 entry: the earlier enablement baseline remains documented in `docs/brainstorms/2026-04-21-portmanager-m3-toward-c-enablement-requirements.md` plus `docs/plans/2026-04-21-portmanager-m3-toward-c-enablement-plan.md`, while the active implementation map is now `docs/brainstorms/2026-04-21-portmanager-m3-review-delta-surface-requirements.md` plus `docs/plans/2026-04-21-portmanager-m3-review-delta-surface-plan.md`. Units 63 through 71 stay landed history with one preserved Debian 12 review packet, artifact coverage `20/20`, `review_open`, and one explicit blocking delta: `container_bridge_transport_substitution` still points at Docker bridge address `172.17.0.2` instead of live Tailscale transport.
- Active execution lane now keeps Milestone 3 inside bounded `Phase 0 enablement` while Milestone 2 stays the guardrail truth surface: continue using `pnpm milestone:review:promotion-ready -- --limit 20`, `pnpm milestone:fetch:review-pack`, `.portmanager/reports/milestone-wording-review.md`, the verification report, and the public development-progress page to keep public wording honest. Units 51 through 62 remain the landed enablement baseline, Unit 63 landed review-packet readiness, Units 64 through 69 preserve one bounded Debian 12 review packet under `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`, and Units 70 through 71 already opened bounded review from that completed packet: `/second-target-policy-pack` now keeps guide coverage complete, artifact coverage at `20/20`, bootstrap, steady-state, backup-and-restore, diagnostics, and rollback parity landed, `decisionState: review_required`, `reviewAdjudication.state: review_open`, and `reviewAdjudication.blockingDeltas` explicit while broader supported-target claims remain locked to Ubuntu. The compatibility-safe `/api/controller` consumer boundary, `audit-review-service`, target-profile registry, `/persistence-decision-pack`, `/consumer-boundary-decision-pack`, `/deployment-boundary-decision-pack`, and `/second-target-policy-pack` stay stable. The Debian 12 governance slice still freezes the docs contract, acceptance recipe, review-packet template, five proof guides, and operator ownership note so review artifacts stay explicit, the preserved packet still names Docker bridge address `172.17.0.2` as the current blocker, and the next work now narrows to one live Tailscale-backed bounded packet or an explicit decision to keep support locked instead of inventing more packet-capture scaffolding. The synced and published readiness state remains `promotion-ready`; exact live counters and the latest qualified run stay on the development-progress page plus the tracked confidence artifact, while Milestone 3 continues without pretending Scheme C is already delivered.
...
- `Next lane`: Milestone 3 `Phase 0 enablement` on top of the same accepted live slice: keep `pnpm milestone:review:promotion-ready -- --limit 20` and `pnpm milestone:fetch:review-pack` as the Milestone 2 wording guardrails, keep the landed `/persistence-decision-pack`, `/consumer-boundary-decision-pack`, `/deployment-boundary-decision-pack`, and `/second-target-policy-pack` review surfaces, target-profile registry, and `/api/controller` consumer boundary stable, and treat `docs/brainstorms/2026-04-21-portmanager-m3-review-delta-surface-requirements.md` plus `docs/plans/2026-04-21-portmanager-m3-review-delta-surface-plan.md` as the current map. Units 63 through 71 stay landed history, the first complete bounded Debian 12 review packet remains preserved at `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`, guide coverage stays complete, artifact coverage is `20/20`, and `/second-target-policy-pack` now also exposes `review_open`, pending adjudication verdicts, and the blocking delta `container_bridge_transport_substitution`. Public snapshot refresh still happens only through the existing helper plus `--refresh-published-artifact` when review agrees, while the next work stays on live Tailscale follow-up for Docker bridge address `172.17.0.2` instead of reopening already-landed seams.
