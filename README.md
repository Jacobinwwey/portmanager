...
- use `pmg` as the primary CLI command prefix while keeping `portmanager` as a compatibility alias during transition
...
- keep review/evidence helpers as publication and high-risk-change guardrails rather than the default blocker for ordinary implementation progress
...
- Real today from Unit 6 and Unit 7: `pnpm milestone:verify:confidence` now composes the standing `pnpm acceptance:verify` gate with the remote-backup replay proof, while `.github/workflows/mainline-acceptance.yml` keeps that heavier confidence routine on explicit `workflow_dispatch` plus the daily scheduled history run instead of making ordinary `push main` progress pay that cost by default.
...
- Real today from the current readiness slice: persisted confidence history now classifies `local-only`, `building-history`, and `promotion-ready`, marks whether each run qualifies for milestone-readiness advancement, and measures progress against one explicit threshold: `7` qualified runs plus `3` consecutive qualified passes from the qualified history scope (`push`, `workflow_dispatch`, or `schedule` on `refs/heads/main`). The active heavy-confidence collection lane, however, now stays limited to explicit `workflow_dispatch` plus the daily scheduled history run.
...
- New direction docs now extend that chain through the full Milestone 2 confidence series and into Milestone 3 entry: the earlier enablement baseline remains documented in `docs/brainstorms/2026-04-21-portmanager-m3-toward-c-enablement-requirements.md` plus `docs/plans/2026-04-21-portmanager-m3-toward-c-enablement-plan.md`, the landed live-follow-up slice is preserved in `docs/brainstorms/2026-04-21-portmanager-m3-live-tailscale-follow-up-requirements.md` plus `docs/plans/2026-04-21-portmanager-m3-live-tailscale-follow-up-plan.md`, the landed discovery slice is now preserved in `docs/brainstorms/2026-04-21-portmanager-m3-live-packet-discovery-requirements.md` plus `docs/plans/2026-04-21-portmanager-m3-live-packet-discovery-plan.md`, the landed review-delta plus execution-tooling groundwork remains preserved in `docs/brainstorms/2026-04-21-portmanager-m3-review-delta-surface-requirements.md`, `docs/plans/2026-04-21-portmanager-m3-review-delta-surface-plan.md`, `docs/brainstorms/2026-04-21-portmanager-m3-live-packet-execution-tooling-requirements.md`, and `docs/plans/2026-04-21-portmanager-m3-live-packet-execution-tooling-plan.md`, the landed source-auto-resolution slice now stays preserved in `docs/brainstorms/2026-04-21-portmanager-m3-live-packet-source-auto-resolution-requirements.md` plus `docs/plans/2026-04-21-portmanager-m3-live-packet-source-auto-resolution-plan.md`, and the active implementation map is now `docs/brainstorms/2026-04-22-portmanager-m3-live-packet-capture-preflight-requirements.md` plus `docs/plans/2026-04-22-portmanager-m3-live-packet-capture-preflight-plan.md`. Units 63 through 79 stay landed history with one preserved Debian 12 review packet, artifact coverage `20/20`, `review_open`, pending adjudication verdicts, landed filesystem-backed discovery, explicit `reviewAdjudication.blockingDeltas`, and repaired remote `main` CI parity after the stale live-loader `hold` expectation moved to `review_required`; the remaining gap is now concrete: blocking delta `container_bridge_transport_substitution` keeps Docker bridge address `172.17.0.2` as public truth until one real live Tailscale packet is committed, `pnpm milestone:preview:live-packet -- --packet-date <date> --controller-base-url <url>` is now the preferred repo-native read-only preflight, and `pnpm milestone:capture:live-packet -- --packet-date <date> --controller-base-url <url>` remains the packet-writing path while explicit ids stay bounded overrides for that packet.
- A smaller closeout slice now sits on top of that baseline: `docs/brainstorms/2026-04-27-portmanager-m2-closeout-incus-runway-requirements.md` plus `docs/plans/2026-04-27-portmanager-m2-closeout-incus-runway-plan.md` freeze one repo-native `pnpm milestone:rehearse:debian12-incus -- --name portmanager-debian12-review` helper and the first frontend runway surfaces so Milestone 2 guardrail discipline plus the bounded Toward C next queue stay visible together.
- Active execution lane now keeps Milestone 3 inside bounded `Phase 0 enablement` while Milestone 2 stays the guardrail truth surface: continue using `pnpm milestone:review:promotion-ready -- --limit 20`, `pnpm milestone:fetch:review-pack`, `.portmanager/reports/milestone-wording-review.md`, the verification report, and the public development-progress page to keep public wording honest. Units 51 through 62 remain the landed enablement baseline, Units 63 through 79 stay landed history, and the next queue is the explicit blocking live-packet capture follow-up: keep the preserved Debian 12 Docker-bridge packet immutable, keep the compatibility-safe `/api/controller` consumer boundary, `audit-review-service`, target-profile registry, `/persistence-decision-pack`, `/consumer-boundary-decision-pack`, `/deployment-boundary-decision-pack`, and `/second-target-policy-pack` stable, surface `container_bridge_transport_substitution` through `/second-target-policy-pack.reviewAdjudication.blockingDeltas`, make `pnpm milestone:preview:live-packet` the preferred bounded read-only preflight, follow with `pnpm milestone:capture:live-packet` only after preview reports ready, and keep `pnpm milestone:scaffold:live-packet`, `pnpm milestone:assemble:live-packet`, and `pnpm milestone:validate:live-packet` as fallback and verification layers while replacing Docker bridge truth `172.17.0.2` with one real committed live Tailscale packet. The synced and published readiness state remains `promotion-ready`; exact live counters and the latest qualified run stay on the development-progress page plus the tracked confidence artifact, while Milestone 3 continues without pretending Scheme C is already delivered.
...
- `Unit 7`: complete. `.github/workflows/mainline-acceptance.yml` now keeps ordinary PR and `push main` validation on `pnpm acceptance:verify`, while `pnpm milestone:verify:confidence` remains on explicit `workflow_dispatch` plus the daily scheduled history lane.
...
- `Next lane`: Milestone 3 `Phase 0 enablement` on top of the same accepted live slice: keep `pnpm milestone:review:promotion-ready -- --limit 20` and `pnpm milestone:fetch:review-pack` as the Milestone 2 wording guardrails, keep the landed `/persistence-decision-pack`, `/consumer-boundary-decision-pack`, `/deployment-boundary-decision-pack`, and `/second-target-policy-pack` review surfaces, target-profile registry, and `/api/controller` consumer boundary stable, and treat `docs/brainstorms/2026-04-22-portmanager-m3-live-packet-capture-preflight-requirements.md` plus `docs/plans/2026-04-22-portmanager-m3-live-packet-capture-preflight-plan.md` as the current map while the earlier source-auto-resolution pair stays landed history. Units 63 through 79 stay landed history, the first complete bounded Debian 12 review packet remains preserved at `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`, guide coverage stays complete, artifact coverage is `20/20`, and `/second-target-policy-pack` now exposes `review_open`, pending adjudication verdicts, `reviewAdjudication.blockingDeltas`, plus `liveTransportFollowUp.state: capture_required` with guide path `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md`; the active work is to keep blocking delta `container_bridge_transport_substitution` and Docker bridge truth `172.17.0.2` visible while `pnpm milestone:preview:live-packet -- --packet-date <date> --controller-base-url <url>` becomes the preferred bounded read-only preflight, `pnpm milestone:capture:live-packet -- --packet-date <date> --controller-base-url <url>` remains the bounded evidence collector after preview reports ready, `--candidate-target-profile-id`, `--host-id`, and `--bootstrap-operation-id` stay bounded overrides, and scaffold, assemble, plus validator helpers stay as fallback and verification layers around one real committed live Tailscale packet.
...
- 当前也已补上 Unit 6 与 Unit 7：`pnpm milestone:verify:confidence` 现在已经把既有 `pnpm acceptance:verify` gate 与 remote-backup replay proof 收敛成一条规范 routine；`.github/workflows/mainline-acceptance.yml` 现在把这条更重的 routine 收敛到显式 `workflow_dispatch` 与每日 schedule 历史路径上，而普通 `push main` 继续只承担较轻的 `acceptance:verify` gate。
...
- 新的方向文档已经在此前完整的 Milestone 2 confidence 文档链之上，继续进入 Milestone 3：此前补上的 `docs/brainstorms/2026-04-21-portmanager-m3-toward-c-enablement-requirements.md` 与 `docs/plans/2026-04-21-portmanager-m3-toward-c-enablement-plan.md` 继续作为 enablement 基线，已落地的 live follow-up 切片保留在 `docs/brainstorms/2026-04-21-portmanager-m3-live-tailscale-follow-up-requirements.md` 与 `docs/plans/2026-04-21-portmanager-m3-live-tailscale-follow-up-plan.md`，已落地的 discovery 切片保留在 `docs/brainstorms/2026-04-21-portmanager-m3-live-packet-discovery-requirements.md` 与 `docs/plans/2026-04-21-portmanager-m3-live-packet-discovery-plan.md`，已落地的 review-delta 与 execution-tooling groundwork 继续保留在 `docs/brainstorms/2026-04-21-portmanager-m3-review-delta-surface-requirements.md`、`docs/plans/2026-04-21-portmanager-m3-review-delta-surface-plan.md`、`docs/brainstorms/2026-04-21-portmanager-m3-live-packet-execution-tooling-requirements.md` 与 `docs/plans/2026-04-21-portmanager-m3-live-packet-execution-tooling-plan.md`，此前的 source auto-resolution 切片也已经成为已落地历史，而当前激活的实现地图已经切到 `docs/brainstorms/2026-04-22-portmanager-m3-live-packet-capture-preflight-requirements.md` 与 `docs/plans/2026-04-22-portmanager-m3-live-packet-capture-preflight-plan.md`。此前公开的 Units 63 through 79 现在已经全部成为已落地历史，artifact coverage 继续保持 `20/20`，remote `main` 的 CI 也已经在把 stale live-loader `hold` 预期修正到 `review_required` 后恢复一致；同时 `/second-target-policy-pack.reviewAdjudication.blockingDeltas` 现在会显式公开阻塞 delta `container_bridge_transport_substitution`：Docker bridge 地址 `172.17.0.2` 仍在替代 live Tailscale transport，所以 `liveTransportFollowUp.state` 继续停在 `capture_required`，支持声明也继续锁定，而 `pnpm milestone:preview:live-packet -- --packet-date <date> --controller-base-url <url>` 现在成为首选 repo-native 只读预检路径，`pnpm milestone:capture:live-packet -- --packet-date <date> --controller-base-url <url>` 则继续作为写入路径，显式 ids 继续保留为有边界 override 参数。
- remote-backup-replay 切片之后，主动执行主线现在已经转向 Milestone 3 的有边界 enablement，同时继续保留 Milestone 2 的 promotion-ready 文案复核与 gate 健康作为真相护栏：继续执行 `pnpm milestone:review:promotion-ready -- --limit 20` 这条 repo-native helper；如果第一问题是当前 CI run，就先执行 `pnpm milestone:fetch:review-pack`，读取 `.portmanager/reports/current-ci-review-pack/` 里的 review pack；随后继续结合带 `Public claim class` 与 `Source surface status` 的 `.portmanager/reports/milestone-wording-review.md`、visibility breakdown、验证报告与公开 docs progress page 做开发者判断。当前这条主线上的 Unit 51 到 Unit 62 继续作为已落地 enablement 基线存在，Units 63 到 79 已经作为已落地历史存在，而下一队列已经切到显式阻塞 delta 跟进：继续把 `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/` 保留为不可变历史 packet，继续把候选 `debian-12-systemd-tailscale` 保持为 `review_required`、`review_open` 与阻塞 delta `container_bridge_transport_substitution` 的真实来源，并把 `pnpm milestone:preview:live-packet` 作为首选 repo-native 只读预检路径，再在 preview 报告 ready 后执行 `pnpm milestone:capture:live-packet`，同时把 `pnpm milestone:scaffold:live-packet`、`pnpm milestone:assemble:live-packet` 与 `pnpm milestone:validate:live-packet` 保留为回退与核验层，而不是继续手工拼接 packet 根目录、重开已落地 seam，或把 execution tooling 本身误写成主缺口。
...
- `Unit 7`：已完成。`.github/workflows/mainline-acceptance.yml` 现在会继续用 `pnpm acceptance:verify` 作为普通 PR 与 `push main` gate，而 `pnpm milestone:verify:confidence` 则保留给显式 `workflow_dispatch` 与每日 schedule 历史路径。
...
- `下一主线`：Milestone 3 现在已经在同一条 accepted live slice 上作为有边界的 `Phase 0 enablement` 打开。继续把 `pnpm milestone:review:promotion-ready -- --limit 20` 与 `pnpm milestone:fetch:review-pack` 当作 Milestone 2 的文案 guardrail，并把 `.portmanager/reports/milestone-wording-review.md`、验证报告与公开 development-progress 页面视为同一份真相包；同时把 `docs/brainstorms/2026-04-22-portmanager-m3-live-packet-capture-preflight-requirements.md` 与 `docs/plans/2026-04-22-portmanager-m3-live-packet-capture-preflight-plan.md` 当作当前实现地图，而此前的 source auto-resolution pair 继续作为已落地历史存在。Units 63 到 79 现在都已经作为已落地历史存在，而第一份完整有边界 Debian 12 review packet 继续保留在 `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`：guide coverage 继续完整、artifact coverage 维持 `20/20`，并且 `/second-target-policy-pack` 现在还会公开 `review_open`、待裁定 verdict、`reviewAdjudication.blockingDeltas` 与 `liveTransportFollowUp.state: capture_required`，同时把 guide path 固定到 `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md`；公开快照仍然只会在现有 helper 与人工复核同意时通过 `--refresh-published-artifact` 前进，而下一步工作已经收窄为继续显式公开 `container_bridge_transport_substitution` 与 Docker bridge 地址 `172.17.0.2`，先执行 `pnpm milestone:preview:live-packet -- --packet-date <date> --controller-base-url <url>` 作为首选只读预检，再执行 `pnpm milestone:capture:live-packet -- --packet-date <date> --controller-base-url <url>` 作为写入路径，把 `--candidate-target-profile-id`、`--host-id`、`--bootstrap-operation-id` 保留为有边界 override，并把 scaffold、assemble 与 validator helper 保留为回退与核验层，为下一份真实 live Tailscale packet 写入证据。
- `当前收尾切片`：在这条 preflight 基线上继续落地 `docs/brainstorms/2026-04-27-portmanager-m2-closeout-incus-runway-requirements.md` 与 `docs/plans/2026-04-27-portmanager-m2-closeout-incus-runway-plan.md`，把 `pnpm milestone:rehearse:debian12-incus -- --name portmanager-debian12-review` 固化成 repo-native Debian 12 预演 helper，并把同一套 Milestone 2 closeout guardrail 加上 Toward C bounded next actions 同步显示到 Web overview 与 docs-site development-progress 前端面。
