...
- remote-backup-replay 切片之后，主动执行主线继续收窄为 Milestone 2 的 confidence-readiness 维护：先用 `pnpm milestone:sync:confidence-history` 同步 completed mainline 证据到本地复核，再用 summary 里的 latest-qualified 区块、visibility breakdown 与公开 docs progress page 做开发者判断，继续把 qualified mainline confidence history 在同一条 live 切片上维持为绿，并由人工根据同步后的 summary 决定里程碑文案是否继续收窄。
