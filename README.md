...
- `Next lane`: Milestone 3 `Phase 0 enablement` on top of the same accepted live slice: keep `pnpm milestone:review:promotion-ready -- --limit 20` and `pnpm milestone:fetch:review-pack` as the Milestone 2 wording guardrails, but direct the next implementation step in `docs/brainstorms/2026-04-21-portmanager-m3-toward-c-enablement-requirements.md` and `docs/plans/2026-04-21-portmanager-m3-toward-c-enablement-plan.md` toward Unit 54 persistence-readiness seams and deeper gateway-ready contract hardening. Public snapshot refresh still happens only through the existing helper plus `--refresh-published-artifact` when review agrees.
