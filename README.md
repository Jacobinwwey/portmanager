...
- Active execution lane now keeps Milestone 3 inside bounded `Phase 0 enablement` while Milestone 2 stays the guardrail truth surface: continue using `pnpm milestone:review:promotion-ready -- --limit 20`, `pnpm milestone:fetch:review-pack`, `.portmanager/reports/milestone-wording-review.md`, the verification report, and the public development-progress page to keep public wording honest. Units 51 through 62 are already landed on that path, including controller seam extraction, the bounded batch envelope, shared indexed event/audit review surfaces, the SQLite-backed persistence adapter, the explicit `/event-audit-index`, `/persistence-readiness`, `/persistence-decision-pack`, `/consumer-boundary-decision-pack`, `/deployment-boundary-decision-pack`, and `/second-target-policy-pack` contract surfaces, the compatibility-safe `/api/controller` consumer boundary across OpenAPI, controller, CLI, and Web, the `audit-review-service` owner behind replay plus indexed review queries, the explicit locked target-profile registry published across controller, CLI, and Web, the consumer-boundary split-criteria pack that keeps `/api/controller` embedded until standalone conditions are real, the deployment-boundary decision pack that keeps standalone review on hold until deployability evidence is real, the second-target policy pack that keeps support locked to Ubuntu while declaring `debian-12-systemd-tailscale` as one review-prep candidate, and the persistence migration-review surface that keeps SQLite active while making cutover pressure explicit. The new governance slice now adds a second-target docs contract, Debian 12 acceptance recipe, Debian 12 review-packet template, Debian 12 bootstrap-proof guide `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`, Debian 12 steady-state proof guide `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`, Debian 12 backup-restore proof guide `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md`, Debian 12 diagnostics proof guide `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md`, and operator ownership note so review-prep artifacts stay explicit, and the new bounded candidate-host create/probe/bootstrap lane lets Debian review-prep start without widening supported-target claims. Bootstrap parity, steady-state parity, backup parity, and diagnostics parity are still not proven. Remaining implementation work now narrows to executing review-packet capture through the bootstrap, steady-state, backup-restore, and diagnostics guides, then proving rollback parity evidence for `debian-12-systemd-tailscale` under `/second-target-policy-pack` while the consumer boundary, target-profile contract, consumer-boundary decision pack, deployment-boundary decision pack, second-target policy pack, and persistence decision pack stay stable. The synced and published readiness state remains `promotion-ready`; exact live counters and the latest qualified run stay on the development-progress page plus the tracked confidence artifact, while Milestone 3 continues without pretending Scheme C is already delivered.
...
- `Next lane`: Milestone 3 `Phase 0 enablement` on top of the same accepted live slice: keep `pnpm milestone:review:promotion-ready -- --limit 20` and `pnpm milestone:fetch:review-pack` as the Milestone 2 wording guardrails, keep the landed `/persistence-decision-pack`, `/consumer-boundary-decision-pack`, `/deployment-boundary-decision-pack`, and `/second-target-policy-pack` review surfaces, target-profile registry, and `/api/controller` consumer boundary stable, keep the new candidate-host create/probe/bootstrap review-prep lane honest, then move next from declared candidate `debian-12-systemd-tailscale` toward the review-packet template at `docs/operations/portmanager-debian-12-review-packet-template.md`, the bootstrap-proof guide at `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`, the steady-state guide at `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`, the backup-restore guide at `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md`, the diagnostics guide at `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md`, and then rollback parity evidence from `docs/brainstorms/2026-04-21-portmanager-m3-toward-c-enablement-requirements.md`, `docs/plans/2026-04-21-portmanager-m3-toward-c-enablement-plan.md`, and the new Debian review-prep docs. Public snapshot refresh still happens only through the existing helper plus `--refresh-published-artifact` when review agrees.
...
- remote-backup-replay 切片之后，主动执行主线现在已经转向 Milestone 3 的有边界 enablement，同时继续保留 Milestone 2 的 promotion-ready 文案复核与 gate 健康作为真相护栏：继续执行 `pnpm milestone:review:promotion-ready -- --limit 20` 这条 repo-native helper；如果第一问题是当前 CI run，就先执行 `pnpm milestone:fetch:review-pack`，读取 `.portmanager/reports/current-ci-review-pack/` 里的 review pack；随后继续结合带 `Public claim class` 与 `Source surface status` 的 `.portmanager/reports/milestone-wording-review.md`、visibility breakdown、验证报告与公开 docs progress page 做开发者判断。当前这条主线上的 Unit 51 到 Unit 62 都已落地，分别覆盖 controller seam extraction、bounded batch envelope、共享 indexed event/audit review surface、SQLite-backed persistence adapter seam、显式 `/event-audit-index`、`/persistence-readiness`、`/persistence-decision-pack`、`/consumer-boundary-decision-pack`、`/deployment-boundary-decision-pack` 与 `/second-target-policy-pack` 合同面、兼容旧路由的 `/api/controller` consumer boundary、统一的 audit-review owner、锁定 Ubuntu 24.04 + systemd + Tailscale 的 target-profile registry、继续让 `/api/controller` 保持内嵌的 consumer-boundary split-criteria pack、继续让独立部署复核保持在 hold 的 deployment-boundary decision pack，以及继续让第二目标支持声明保持在 hold 的 second-target policy pack；现在还额外落地了 candidate host 注册、probe 与 bootstrap 的 review-prep lane，并把 Debian 12 review-packet template、bootstrap-proof guide `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`、steady-state guide `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`、backup-restore guide `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md` 与 diagnostics guide `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md` 明确写入治理文档，但这仍然不代表 bootstrap parity、steady-state parity、backup parity 或 diagnostics parity 已通过。同步后的本地与公开 readiness 都已经进入 `promotion-ready`，而精确实时计数与最新 qualified run 继续留在 development-progress 页面与被跟踪 confidence artifact。后续实现则收窄为围绕 `/second-target-policy-pack` 的 review packet 落盘、通过四份 proof guide 执行 bootstrap、steady-state、backup/restore、diagnostics 证明，并继续推进 rollback 等价证据，而不是继续补 readiness 脚手架或重新打开 consumer route split。
