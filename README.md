...
- `下一主线`：继续在同一条 live 切片上推进 Milestone 2 的 promotion-ready 文案复核：先执行 `pnpm milestone:review:promotion-ready -- --limit 20`，只在人工复核同意时通过同一条 helper 加上 `--refresh-published-artifact` 推进公开快照，结合验证报告与公开 development-progress 页面做开发者复核，并把 qualified history 持续转绿。
