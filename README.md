...
- Still missing: CLI host/rule/policy workflows, live web-controller data wiring for the full V1 information architecture, and a long-lived agent `HTTP over Tailscale` steady-state service.
...
- Active execution lane is now Unit 2 CLI parity plus Unit 3 Web live-data parity; Unit 1 controller parity is complete and Unit 0 remains mandatory.
...
- `Unit 2`: mirror those controller resources into Rust CLI read and write flows so CLI becomes a truthful peer to controller contracts.
...
当前分支已经加入 Milestone 1 的基础实现代码，包括 workspace、契约生成链路、controller 服务骨架、controller 侧本地备份、回滚、诊断、快照与事件流原语、真实 controller `hosts` / `bridge-rules` / `exposure-policies` 资源与 host probe / bootstrap 路径、首条可运行的 Rust CLI read path 与 event history 命令、文件落盘式 Rust agent 骨架、overview / host detail 的 React SPA shell，以及单主机 / 单规则 / 诊断 / 单回滚验证脚本，同时保留 `GitHub Pages + VitePress` 文档发布层。
...
- 仍然缺失：CLI 的 host/rule/policy 工作流，覆盖完整 V1 信息架构的 live web-controller 数据链路，以及长驻式的 agent `HTTP over Tailscale` 稳态服务。
...
- 当前主动执行主线已经转到 Unit 2 CLI 一致性与 Unit 3 Web live-data 一致性；Unit 1 controller 表面一致性已经完成，Unit 0 仍然必须保持。
...
- `Unit 2`：把这些资源同步到 Rust CLI 的读写工作流，让 CLI 真正成为与 controller 契约一致的对等入口。
