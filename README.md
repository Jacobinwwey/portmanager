...
- Still missing: the long-lived agent `HTTP over Tailscale` steady-state service plus the final Unit 5 acceptance/doc resync pass.
...
- Active execution lane is now Unit 4 agent steady-state integration plus Unit 5 acceptance/doc resync; Units 1 through 3 are complete and Unit 0 remains mandatory.
...
- 仍然缺失：长驻式的 agent `HTTP over Tailscale` 稳态服务，以及最后的 Unit 5 验收 / 文档重同步。
- 当前 Web 状态：live controller-backed 路由一致性已经真实落地；在没有配置 controller base URL 时，mock factory 只作为预览回退存在。
- 当前主动执行主线已经转到 Unit 4 agent 稳态集成与 Unit 5 验收 / 文档重同步；Unit 1 到 Unit 3 已经完成，Unit 0 仍然必须保持。
