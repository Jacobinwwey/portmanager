...
- [ ] Next lane: keep Milestone 3 as bounded `Phase 0 enablement` while Milestone 2 review helpers remain the guardrail truth surface. Continue running `pnpm milestone:review:promotion-ready -- --limit 20` after completed mainline runs, use `pnpm milestone:fetch:review-pack` when the current CI run is the first question, keep `.portmanager/reports/milestone-wording-review.md`, `Public claim class`, `Source surface status`, the verification report, and the public development-progress page as the wording-truth bundle, keep the preserved Units 64-69 review packet at `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/` honest, treat landed Units 74 through 79 plus source auto-resolution as landed history, and narrow the active implementation map to `docs/brainstorms/2026-04-22-portmanager-m3-live-packet-capture-preflight-requirements.md` plus `docs/plans/2026-04-22-portmanager-m3-live-packet-capture-preflight-plan.md`: keep `review_open`, pending verdicts, and `reviewAdjudication.blockingDeltas` honest, keep blocking delta `container_bridge_transport_substitution` plus Docker bridge truth `172.17.0.2` visible, make `pnpm milestone:preview:live-packet -- --packet-date <date> --controller-base-url <url>` the preferred bounded read-only preflight, follow with `pnpm milestone:capture:live-packet -- --packet-date <date> --controller-base-url <url>` when preview reports ready, keep `--candidate-target-profile-id`, `--host-id`, and `--bootstrap-operation-id` as bounded overrides, and keep scaffold, assemble, plus validator helpers as fallback and verification layers without reopening `/consumer-boundary-decision-pack`, `/deployment-boundary-decision-pack`, `/persistence-decision-pack`, `/second-target-policy-pack`, the target-profile registry, or `/api/controller`.
- [x] Closeout slice: land `docs/brainstorms/2026-04-27-portmanager-m2-closeout-incus-runway-requirements.md` plus `docs/plans/2026-04-27-portmanager-m2-closeout-incus-runway-plan.md`, add repo-native `pnpm milestone:rehearse:debian12-incus -- --name portmanager-debian12-review`, and surface the same Milestone 2 guardrail plus Toward C bounded runway on Web overview and docs-site development-progress pages.
...
- [x] Land Milestone 3 live packet discovery requirements doc: `docs/brainstorms/2026-04-21-portmanager-m3-live-packet-discovery-requirements.md`
- [x] Land Milestone 3 live packet discovery implementation plan: `docs/plans/2026-04-21-portmanager-m3-live-packet-discovery-plan.md`
- [x] Land Milestone 3 live packet execution tooling requirements doc: `docs/brainstorms/2026-04-21-portmanager-m3-live-packet-execution-tooling-requirements.md`
- [x] Land Milestone 3 live packet execution tooling implementation plan: `docs/plans/2026-04-21-portmanager-m3-live-packet-execution-tooling-plan.md`
- [x] Land Milestone 2 closeout and incus runway requirements doc: `docs/brainstorms/2026-04-27-portmanager-m2-closeout-incus-runway-requirements.md`
- [x] Land Milestone 2 closeout and incus runway implementation plan: `docs/plans/2026-04-27-portmanager-m2-closeout-incus-runway-plan.md`
...
- [ ] 下一主线：继续把 Milestone 3 保持为有边界的 `Phase 0 enablement`，同时把 Milestone 2 的 review helper 保留为 guardrail 真相面。继续在 completed mainline runs 之后执行 `pnpm milestone:review:promotion-ready -- --limit 20`；如果第一问题是当前 CI run，就先执行 `pnpm milestone:fetch:review-pack` 并读取 `.portmanager/reports/current-ci-review-pack/`；继续把 `.portmanager/reports/milestone-wording-review.md`、`Public claim class`、`Source surface status`、验证报告与公开 development-progress 页面当作文案真相包；继续把已经保留到 `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/` 的 Units 64-69 review packet 保持诚实；把已落地的 Units 74 到 79 与 source auto-resolution 当作历史基线；然后把后续实现收窄到 `docs/brainstorms/2026-04-22-portmanager-m3-live-packet-capture-preflight-requirements.md` 与 `docs/plans/2026-04-22-portmanager-m3-live-packet-capture-preflight-plan.md`：继续把 `review_open`、待裁定 verdict 与 `reviewAdjudication.blockingDeltas` 保持诚实，继续把阻塞 delta `container_bridge_transport_substitution` 与 Docker bridge 真相 `172.17.0.2` 公开出来，并把 `pnpm milestone:preview:live-packet -- --packet-date <date> --controller-base-url <url>` 当作首选 repo-native 只读预检，再在 preview 报告 ready 后执行 `pnpm milestone:capture:live-packet -- --packet-date <date> --controller-base-url <url>`，把 `--candidate-target-profile-id`、`--host-id` 与 `--bootstrap-operation-id` 保留为有边界 override，把 `pnpm milestone:scaffold:live-packet`、`pnpm milestone:assemble:live-packet` 与 `pnpm milestone:validate:live-packet` 保留为回退与核验层，而不是重新打开 `/consumer-boundary-decision-pack`、`/deployment-boundary-decision-pack`、`/persistence-decision-pack`、`/second-target-policy-pack`、target-profile registry 或 `/api/controller` 的已落地 seam。
- [x] 当前收尾切片：在这条 preflight 基线上继续落地 `docs/brainstorms/2026-04-27-portmanager-m2-closeout-incus-runway-requirements.md` 与 `docs/plans/2026-04-27-portmanager-m2-closeout-incus-runway-plan.md`，把 `pnpm milestone:rehearse:debian12-incus -- --name portmanager-debian12-review` 固化成 repo-native Debian 12 预演 helper，并把同一套 Milestone 2 closeout guardrail 加上 Toward C bounded next actions 同步显示到 Web overview 与 docs-site development-progress 前端面。
...
- [x] 落盘 Milestone 3 live packet discovery 需求文档：`docs/brainstorms/2026-04-21-portmanager-m3-live-packet-discovery-requirements.md`
- [x] 落盘 Milestone 3 live packet discovery 实现计划：`docs/plans/2026-04-21-portmanager-m3-live-packet-discovery-plan.md`
- [x] 落盘 Milestone 3 live packet execution tooling 需求文档：`docs/brainstorms/2026-04-21-portmanager-m3-live-packet-execution-tooling-requirements.md`
- [x] 落盘 Milestone 3 live packet execution tooling 实现计划：`docs/plans/2026-04-21-portmanager-m3-live-packet-execution-tooling-plan.md`
- [x] 落盘 Milestone 3 live packet capture automation 需求文档：`docs/brainstorms/2026-04-21-portmanager-m3-live-packet-capture-automation-requirements.md`
- [x] 落盘 Milestone 3 live packet capture automation 实现计划：`docs/plans/2026-04-21-portmanager-m3-live-packet-capture-automation-plan.md`
- [x] 落盘 Milestone 3 live packet source auto-resolution 需求文档：`docs/brainstorms/2026-04-21-portmanager-m3-live-packet-source-auto-resolution-requirements.md`
- [x] 落盘 Milestone 3 live packet source auto-resolution 实现计划：`docs/plans/2026-04-21-portmanager-m3-live-packet-source-auto-resolution-plan.md`
- [x] 落盘 Milestone 3 live packet capture preflight 需求文档：`docs/brainstorms/2026-04-22-portmanager-m3-live-packet-capture-preflight-requirements.md`
- [x] 落盘 Milestone 3 live packet capture preflight 实现计划：`docs/plans/2026-04-22-portmanager-m3-live-packet-capture-preflight-plan.md`
- [x] 落盘 Milestone 2 closeout and incus runway 需求文档：`docs/brainstorms/2026-04-27-portmanager-m2-closeout-incus-runway-requirements.md`
- [x] 落盘 Milestone 2 closeout and incus runway 实现计划：`docs/plans/2026-04-27-portmanager-m2-closeout-incus-runway-plan.md`
