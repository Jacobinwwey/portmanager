...
- [ ] Next lane: harden Milestone 2 reliability on the same live slice with repeated proof across configured, failed, and local-only remote-backup evidence.
...
- [ ] 下一主线：继续在同一条 live 切片上加固 Milestone 2 可靠性，重点放在 configured、failed、local-only 三类 remote-backup 证据的重复证明。
