...
fn format_second_target_bootstrap_proof_capture_block(bootstrap_proof_capture: &Value) -> String {
    ...
    let source_lines = bootstrap_proof_capture["sources"]
        .as_array()
        .into_iter()
        .flatten()
        .filter_map(|source| source.as_str())
    ...
    format!(
        "Bootstrap Proof Capture:\nCandidate Target: {}\nGuide Path: {}\nSummary: {}\n{}\n{}",
        bootstrap_proof_capture["candidateTargetProfileId"]
            .as_str()
            .unwrap_or("unknown"),
        bootstrap_proof_capture["guidePath"]
            .as_str()
            .unwrap_or("unknown"),
        bootstrap_proof_capture["summary"]
            .as_str()
            .unwrap_or("no summary"),
        required_block,
        source_block
...
fn format_second_target_steady_state_proof_capture_block(steady_state_proof_capture: &Value) -> String {
    ...
    let source_lines = steady_state_proof_capture["sources"]
        .as_array()
        .into_iter()
        .flatten()
        .filter_map(|source| source.as_str())
    ...
    format!(
        "Steady-State Proof Capture:\nCandidate Target: {}\nGuide Path: {}\nSummary: {}\n{}\n{}",
        steady_state_proof_capture["candidateTargetProfileId"]
            .as_str()
            .unwrap_or("unknown"),
        steady_state_proof_capture["guidePath"]
            .as_str()
            .unwrap_or("unknown"),
        steady_state_proof_capture["summary"]
            .as_str()
            .unwrap_or("no summary"),
        required_block,
        source_block
...
fn format_second_target_backup_restore_proof_capture_block(backup_restore_proof_capture: &Value) -> String {
    let required_lines = backup_restore_proof_capture["requiredArtifacts"]
        .as_array()
        .into_iter()
        .flatten()
        .map(|item| {
    ...
    let source_lines = backup_restore_proof_capture["sources"]
        .as_array()
        .into_iter()
        .flatten()
        .filter_map(|source| source.as_str())
    ...
    format!(
        "Backup and Restore Proof Capture:\nCandidate Target: {}\nGuide Path: {}\nSummary: {}\n{}\n{}",
        backup_restore_proof_capture["candidateTargetProfileId"]
            .as_str()
            .unwrap_or("unknown"),
        backup_restore_proof_capture["guidePath"]
            .as_str()
            .unwrap_or("unknown"),
        backup_restore_proof_capture["summary"]
            .as_str()
            .unwrap_or("no summary"),
        required_block,
        source_block
...
fn format_second_target_rollback_proof_capture_block(rollback_proof_capture: &Value) -> String {
    let required_lines = rollback_proof_capture["requiredArtifacts"]
        .as_array()
        .into_iter()
        .flatten()
        .map(|item| {
    ...
    let source_lines = rollback_proof_capture["sources"]
        .as_array()
        .into_iter()
        .flatten()
        .filter_map(|source| source.as_str())
    ...
    format!(
        ...
        rollback_proof_capture["candidateTargetProfileId"]
            .as_str()
            .unwrap_or("unknown"),
        rollback_proof_capture["guidePath"]
            .as_str()
            .unwrap_or("unknown"),
        rollback_proof_capture["summary"]
            .as_str()
            .unwrap_or("no summary"),
        required_block,
        source_block
...
fn format_second_target_policy_pack_text(pack: &Value) -> String {
    ...
    format!(
        ...
        format_second_target_rollback_proof_capture_block(&pack["rollbackProofCapture"]),
        format_decision_criteria_block("Satisfied Criteria", &pack["satisfiedCriteria"]),
        format_decision_criteria_block("Blocking Criteria", &pack["blockingCriteria"])
    )
}
