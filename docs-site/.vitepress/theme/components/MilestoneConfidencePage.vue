...
const currentDirectionSummary = computed(() => props.locale === 'zh'
  ? progress.readiness.status === 'promotion-ready'
    ? [
        ...
        'Milestone 3 已经作为有边界的 `Phase 0 enablement` 打开；Unit 51 到 Unit 62 已经落地，其中包含已落地的 Unit 57 audit/event boundary、Unit 58 target-profile abstraction rule、Unit 60 consumer-boundary split criteria、Unit 61 deployment-boundary decision pack、Unit 62 second-target policy pack，以及 `/consumer-boundary-decision-pack`、`/deployment-boundary-decision-pack`、`/persistence-decision-pack` 与 `/second-target-policy-pack` 四类 review surface，但当前代码仍然没有 PostgreSQL backend 路径，也没有超出锁定画像的第二目标支持声明。',
        `当前最新 qualified 主线 run 为 ${latestQualifiedRunLabel.value}；新的实现能量现在已经从 Unit 62 交付转向围绕 \`/second-target-policy-pack\` 的候选第二目标声明、等价证据、文档契约、验收 recipe 与 operator ownership 证明，同时继续把已落地的 audit/event boundary、target-profile abstraction rule、consumer-boundary split criteria、deployment-boundary decision pack、second-target policy pack 与 persistence promotion decision pack 作为稳定基线。`
      ]
    : [
