...
const currentDirectionSummary = computed(() => props.locale === 'zh'
  ? progress.readiness.status === 'promotion-ready'
    ? [
        ...
        `当前最新 qualified 主线 run 为 ${latestQualifiedRunLabel.value}；当前实现地图已经切到 \`docs/brainstorms/2026-04-21-portmanager-m3-live-packet-discovery-requirements.md\` 与 \`docs/plans/2026-04-21-portmanager-m3-live-packet-discovery-plan.md\`：Units 63 到 73 现已全部成为已落地历史，\`debian-12-systemd-tailscale\` 在 \`/second-target-policy-pack\` 下继续公开 guide coverage \`6/6\`、artifact coverage \`20/20\`、\`packet_ready\`、\`review_required\`、\`review_open\` 与顶层 \`liveTransportFollowUp.state: capture_required\`，remote \`main\` CI 也已经在把 stale live-loader \`hold\` 预期修正到 \`review_required\` 后恢复一致；当前剩余缺口已经收窄为通过 \`docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md\` 与 \`docs/operations/portmanager-debian-12-review-packet-template.md\` 固定 \`live-transport-follow-up-summary.json\`，再让 controller 默认真相自动发现最新有效 live packet，而当前保留的 Docker bridge 地址仍是 \`172.17.0.2\`。`
      ]
    : [
  ...
  : progress.readiness.status === 'promotion-ready'
    ? [
        ...
        `The latest qualified mainline run is ${latestQualifiedRunLabel.value}; the active implementation map now shifts to \`docs/brainstorms/2026-04-21-portmanager-m3-live-packet-discovery-requirements.md\` and \`docs/plans/2026-04-21-portmanager-m3-live-packet-discovery-plan.md\`: Units 63 through 73 are now landed history, \`debian-12-systemd-tailscale\` still publishes guide coverage \`6/6\`, artifact coverage \`20/20\`, \`packet_ready\`, \`review_required\`, \`review_open\`, and top-level \`liveTransportFollowUp.state: capture_required\` through \`/second-target-policy-pack\`, remote \`main\` CI parity is repaired after the stale live-loader \`hold\` expectation moved to \`review_required\`, and the remaining gap has collapsed into filesystem-backed live packet discovery: standardize \`live-transport-follow-up-summary.json\` in the live follow-up guide plus review-packet template, then let controller default truth discover the newest valid packet root while the preserved Docker bridge address stays \`172.17.0.2\` and broader second-target support remains locked.`
      ]
    : [
...
const currentDirectionDocs = computed(() => [
  {
    href: githubSourceLink('docs/brainstorms/2026-04-21-portmanager-m3-live-packet-discovery-requirements.md'),
    label: copy.value.currentDirectionRequirementsLink
  },
  {
    href: githubSourceLink('docs/plans/2026-04-21-portmanager-m3-live-packet-discovery-plan.md'),
