...
const currentDirectionSummary = computed(() => props.locale === 'zh'
  ? progress.readiness.status === 'promotion-ready'
    ? [
        ...
        `当前最新 qualified 主线 run 为 ${latestQualifiedRunLabel.value}；当前实现地图已经切到 \`docs/brainstorms/2026-04-21-portmanager-m3-review-delta-surface-requirements.md\` 与 \`docs/plans/2026-04-21-portmanager-m3-review-delta-surface-plan.md\`：Units 63 到 76 现已全部成为已落地历史，\`debian-12-systemd-tailscale\` 在 \`/second-target-policy-pack\` 下继续公开 guide coverage \`6/6\`、artifact coverage \`20/20\`、\`packet_ready\`、\`review_required\`、\`review_open\`、\`reviewAdjudication.blockingDeltas\` 与顶层 \`liveTransportFollowUp.state: capture_required\`，remote \`main\` CI 也已经在把 stale live-loader \`hold\` 预期修正到 \`review_required\` 后恢复一致；当前剩余缺口已经收窄为继续公开阻塞 delta \`container_bridge_transport_substitution\` 与 Docker bridge 真相 \`172.17.0.2\`，再通过 \`pnpm milestone:scaffold:live-packet\`、\`pnpm milestone:assemble:live-packet\` 与 \`pnpm milestone:validate:live-packet\` 为下一份真实 live Tailscale packet 写入证据，同时继续沿用 \`docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md\`、\`docs/operations/portmanager-debian-12-review-packet-template.md\` 与 \`live-transport-follow-up-summary.json\` 契约。`
      ]
    : [
  ...
  : progress.readiness.status === 'promotion-ready'
    ? [
        ...
        `The latest qualified mainline run is ${latestQualifiedRunLabel.value}; the active implementation map now shifts to \`docs/brainstorms/2026-04-21-portmanager-m3-review-delta-surface-requirements.md\` and \`docs/plans/2026-04-21-portmanager-m3-review-delta-surface-plan.md\`: Units 63 through 76 are now landed history, \`debian-12-systemd-tailscale\` still publishes guide coverage \`6/6\`, artifact coverage \`20/20\`, \`packet_ready\`, \`review_required\`, \`review_open\`, \`reviewAdjudication.blockingDeltas\`, and top-level \`liveTransportFollowUp.state: capture_required\` through \`/second-target-policy-pack\`, remote \`main\` CI parity is repaired after the stale live-loader \`hold\` expectation moved to \`review_required\`, and the remaining gap has collapsed into one explicit blocking delta: keep \`container_bridge_transport_substitution\` and preserved Docker bridge truth \`172.17.0.2\` visible while \`pnpm milestone:scaffold:live-packet\`, \`pnpm milestone:assemble:live-packet\`, and \`pnpm milestone:validate:live-packet\` prepare one real committed live Tailscale packet and broader second-target support stays locked.`
      ]
    : [
