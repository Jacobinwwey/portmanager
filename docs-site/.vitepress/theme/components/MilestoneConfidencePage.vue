...
const currentDirectionSummary = computed(() => props.locale === 'zh'
  ? progress.readiness.status === 'promotion-ready'
    ? [
        ...
        'Milestone 3 已经作为有边界的 `Phase 0 enablement` 打开；Unit 51 到 Unit 60 已经落地，其中包含已落地的 Unit 57 audit/event boundary、Unit 58 target-profile abstraction rule、Unit 60 consumer-boundary split criteria，以及 `/consumer-boundary-decision-pack` 与 `/persistence-decision-pack` 两类 review surface，但当前代码仍然没有 PostgreSQL backend 路径，也没有超出锁定画像的第二目标支持声明。',
        `当前最新 qualified 主线 run 为 ${latestQualifiedRunLabel.value}；新的实现能量现在已经从 Unit 60 交付转向更后置的独立部署边界证据与第二目标策略工作，同时继续把已落地的 audit/event boundary、target-profile abstraction rule、consumer-boundary split criteria 与 persistence promotion decision pack 作为稳定基线。`
      ]
    : [
  ...
  : progress.readiness.status === 'promotion-ready'
    ? [
        ...
        'Milestone 3 is now open as bounded `Phase 0 enablement`; Units 51 through 60 are real now, including the landed Unit 57 audit/event boundary decisions, Unit 58 target-profile abstraction rules, Unit 60 consumer-boundary split criteria, and the new `/consumer-boundary-decision-pack` plus `/persistence-decision-pack` review surfaces, but current code still lacks a PostgreSQL backend path and any broader second-target support claim.',
        `The latest qualified mainline run is ${latestQualifiedRunLabel.value}; new implementation energy has now shifted beyond Unit 60 toward later standalone deployment-boundary evidence and broader second-target policy work while the landed audit/event boundary, target-profile abstraction rule, consumer-boundary split criteria, and persistence promotion decision pack stay stable.`
      ]
    : [
