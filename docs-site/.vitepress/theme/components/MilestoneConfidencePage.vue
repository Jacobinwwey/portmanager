...
const currentDirectionSummary = computed(() => props.locale === 'zh'
  ? progress.readiness.status === 'promotion-ready'
    ? [
        ...
        'Milestone 3 已经作为有边界的 `Phase 0 enablement` 打开；Unit 51 到 Unit 62 已经落地，其中包含已落地的 Unit 57 audit/event boundary、Unit 58 target-profile abstraction rule、Unit 60 consumer-boundary split criteria、Unit 61 deployment-boundary decision pack、Unit 62 second-target policy pack，以及 `/consumer-boundary-decision-pack`、`/deployment-boundary-decision-pack`、`/persistence-decision-pack` 与 `/second-target-policy-pack` 四类 review surface。`/second-target-policy-pack` 现在还把 `debian-12-systemd-tailscale` 声明为 review-prep 候选，但当前代码仍然没有 PostgreSQL backend 路径，也没有超出锁定画像的第二目标支持声明。',
        `当前最新 qualified 主线 run 为 ${latestQualifiedRunLabel.value}；新的实现能量现在已经从候选第二目标声明推进到围绕 \`debian-12-systemd-tailscale\` 的传输等价、恢复等价、文档契约、验收 recipe 与 operator ownership 证明，同时继续把已落地的 audit/event boundary、target-profile abstraction rule、consumer-boundary split criteria、deployment-boundary decision pack、second-target policy pack 与 persistence promotion decision pack 作为稳定基线。`
      ]
    : [
  ...
  : progress.readiness.status === 'promotion-ready'
    ? [
        ...
        'Milestone 3 is now open as bounded `Phase 0 enablement`; Units 51 through 62 are real now, including the landed Unit 57 audit/event boundary decisions, Unit 58 target-profile abstraction rules, Unit 60 consumer-boundary split criteria, Unit 61 deployment-boundary decision pack, Unit 62 second-target policy pack, and the new `/consumer-boundary-decision-pack`, `/deployment-boundary-decision-pack`, `/persistence-decision-pack`, plus `/second-target-policy-pack` review surfaces. `/second-target-policy-pack` now also declares `debian-12-systemd-tailscale` as a review-prep candidate while current code still lacks a PostgreSQL backend path and any broader second-target support claim.',
        `The latest qualified mainline run is ${latestQualifiedRunLabel.value}; new implementation energy has now shifted beyond candidate declaration toward transport parity, recovery parity, docs contract, acceptance recipe, and operator ownership proof for \`debian-12-systemd-tailscale\` while the landed audit/event boundary, target-profile abstraction rule, consumer-boundary split criteria, deployment-boundary decision pack, second-target policy pack, and persistence promotion decision pack stay stable.`
      ]
    : [
