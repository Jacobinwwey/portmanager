...
const copy = computed(() => props.locale === 'zh'
  ? {
      ...
      liveConfidenceBody: 'Roadmap 首页现在直接显示已经进入 `promotion-ready` 的公开进度快照，开发者无需离开页面就能看到最新 qualified 主线证据、最新可见 run、visibility-only 噪声拆分，以及默认复核链路会先经过 `pnpm milestone:review:confidence`、再按需显式刷新公开 artifact 的事实。',
  ...
  : {
      ...
      liveConfidenceBody: 'The roadmap home page now exposes the published `promotion-ready` confidence snapshot directly, so developers can see the latest qualified mainline evidence, latest visible run, visibility-only noise split, and the fact that the default review chain runs `pnpm milestone:review:confidence` before any explicit artifact refresh.',
      liveConfidenceReadiness: 'Readiness',
      liveQualifiedRuns: 'Qualified runs:',
      liveQualifiedPasses: 'Qualified consecutive passes:',
      liveRemainingQualifiedRuns: 'Remaining qualified runs:',
