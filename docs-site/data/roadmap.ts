  ...
  buckets: [
    {
      ...
      items: {
        en: [
          ...
          'Milestone 3 Units 51 through 61 are now real: controller seam extraction baseline, bounded batch exposure-policy envelope, explicit `/event-audit-index`, `/persistence-readiness`, `/persistence-decision-pack`, `/consumer-boundary-decision-pack`, and `/deployment-boundary-decision-pack` generated contracts, shared CLI/Web review parity, a SQLite-backed persistence adapter that measures PostgreSQL readiness from live store counts, a compatibility-safe `/api/controller` consumer boundary, one explicit `audit-review-service` owner behind replay plus indexed review, one explicit locked target-profile registry published across controller, CLI, and Web, one consumer-boundary split-criteria pack that keeps `/api/controller` embedded until standalone conditions are real, one deployment-boundary decision pack that keeps standalone review on hold until deployability evidence is real, and one migration-review surface that keeps SQLite active while making cutover pressure explicit.',
          'The active plan now shifts the concrete next queue for developers beyond Unit 61 toward broader second-target policy work while the locked target-profile registry, consumer-boundary decision pack, deployment-boundary decision pack, and persistence decision pack stay the truthful platform contract.',
        ...
        zh: [
          ...
          'Milestone 3 的 Unit 51 到 Unit 61 现在都已真实落地：controller seam extraction baseline、bounded batch exposure-policy envelope、显式 `/event-audit-index`、`/persistence-readiness`、`/persistence-decision-pack`、`/consumer-boundary-decision-pack` 与 `/deployment-boundary-decision-pack` 生成契约、共享 CLI/Web 复核一致性、会从真实 store 计数衡量 PostgreSQL readiness 的 SQLite-backed persistence adapter、兼容旧路由的 `/api/controller` consumer boundary、统一接管 replay 与 indexed review 查询的 `audit-review-service` owner、已经在 controller、CLI、Web 发布的锁定 target-profile registry、明确 `/api/controller` 继续内嵌条件的 consumer-boundary split-criteria pack、明确独立部署复核仍然不能开启条件的 deployment-boundary decision pack，以及继续让 SQLite 保持 active 的 migration-review surface 都已经建立，而且没有改动已验收 evidence model。',
          '当前实现地图也已经把开发者的下一批具体工作转向 Unit 61 之后更聚焦的第二目标策略工作，同时继续把锁定 target-profile registry、consumer-boundary decision pack、deployment-boundary decision pack 与 persistence decision pack 保持为稳定平台契约。',
    ...
    },
    {
      ...
      items: {
        en: [
          ...
          'Units 51 through 61 are now landed on that map, including the new consumer-boundary split criteria, deployment-boundary decision pack, and persistence promotion decision pack.',
          'Move next toward broader second-target policy work instead of reopening already-landed seams or pretending standalone deployment pressure already exists.',
        ...
        zh: [
          ...
          'Unit 51 到 Unit 61 现在都已落地，其中已经包含新的 consumer-boundary split criteria、deployment-boundary decision pack 与 persistence promotion decision pack。',
          '下一步转向更聚焦的第二目标策略，而不是重新打开已经落地的 seam，或假装独立部署压力已经真实出现。',
...
export const roadmapMilestones: RoadmapMilestone[] = [
  ...
  },
  {
    ...
    engineeringWork: {
      en: ['Keep the landed audit-review service boundary stable behind `/events`, `/operations/events`, and `/event-audit-index`.', 'Keep the explicit target-profile registry as the single-source contract for the locked Ubuntu 24.04 + systemd + Tailscale assumption.', 'Keep `/consumer-boundary-decision-pack`, `/deployment-boundary-decision-pack`, and `/persistence-decision-pack` aligned with CLI/Web review parity while broader second-target policy work is defined.'],
      zh: ['继续把已经落地的 audit-review service boundary 稳定保持在 `/events`、`/operations/events` 与 `/event-audit-index` 后方。', '继续把显式 target-profile registry 保持为锁定 Ubuntu 24.04 + systemd + Tailscale 假设的单一契约源。', '继续让 `/consumer-boundary-decision-pack`、`/deployment-boundary-decision-pack` 与 `/persistence-decision-pack` 保持和 CLI/Web 复核一致，同时定义更聚焦的第二目标策略。']
    },
    entryCriteria: {
    ...
    developerFocus: {
      en: [
        'Use `docs/brainstorms/2026-04-21-portmanager-m3-toward-c-enablement-requirements.md` and `docs/plans/2026-04-21-portmanager-m3-toward-c-enablement-plan.md` as the active implementation map; Units 51 through 61 are landed on that path.',
        'Move next to broader second-target policy work instead of reopening routing split, replay ownership, or already-landed target-profile and decision-pack seams.',
      ...
      zh: [
        '把 `docs/brainstorms/2026-04-21-portmanager-m3-toward-c-enablement-requirements.md` 与 `docs/plans/2026-04-21-portmanager-m3-toward-c-enablement-plan.md` 当作当前实现地图；其中 Unit 51 到 Unit 61 已经全部落地。',
