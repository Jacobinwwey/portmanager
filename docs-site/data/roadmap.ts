  ...
  buckets: [
    {
      ...
      items: {
        en: [
          ...
          'The active implementation map now lives in `docs/brainstorms/2026-04-21-portmanager-m3-live-packet-execution-tooling-requirements.md` and `docs/plans/2026-04-21-portmanager-m3-live-packet-execution-tooling-plan.md`: guide coverage is complete, Units 63 through 76 are landed history, `/second-target-policy-pack` still reports `review_open`, blocking delta `container_bridge_transport_substitution`, and top-level `liveTransportFollowUp.state: capture_required`, remote `main` CI parity is repaired after the stale live-loader `hold` expectation moved to `review_required`, the preserved Docker bridge address is still `172.17.0.2`, and the active bounded queue is now repo-native packet execution tooling through `pnpm milestone:scaffold:live-packet`, `pnpm milestone:assemble:live-packet`, `pnpm milestone:validate:live-packet`, and the same `live-transport-follow-up-summary.json` contract while the locked target-profile registry, consumer-boundary decision pack, deployment-boundary decision pack, second-target policy pack, persistence decision pack, and Debian review-prep governance docs stay the truthful platform contract.',
        ...
        zh: [
          ...
          '当前激活的实现地图已经切到 `docs/brainstorms/2026-04-21-portmanager-m3-live-packet-execution-tooling-requirements.md` 与 `docs/plans/2026-04-21-portmanager-m3-live-packet-execution-tooling-plan.md`：guide coverage 已完整、Units 63 到 76 已成为已落地历史，`/second-target-policy-pack` 现在还会公开 `review_open`、阻塞 delta `container_bridge_transport_substitution` 与顶层 `liveTransportFollowUp.state: capture_required`，remote `main` CI 也已经在把 stale live-loader `hold` 预期修正到 `review_required` 后恢复一致，当前保留的 Docker bridge 地址仍然是 `172.17.0.2`，而当前激活队列已经收窄为通过 `pnpm milestone:scaffold:live-packet`、`pnpm milestone:assemble:live-packet`、`pnpm milestone:validate:live-packet` 与同一份 `live-transport-follow-up-summary.json` 契约推进 repo-native packet execution tooling，同时继续让锁定 target-profile registry、consumer-boundary decision pack、deployment-boundary decision pack、second-target policy pack、persistence decision pack 与 Debian review-prep 治理文档保持为稳定平台契约。',
    ...
    },
    {
      ...
      items: {
        ...
        zh: [
          ...
          '索引化 event/audit review、显式 audit-review owner、target-profile registry 发布、persistence-readiness reporting、新的 `/persistence-decision-pack`、`/consumer-boundary-decision-pack` 与 `/deployment-boundary-decision-pack` 复核表面，以及兼容旧路由的 `/api/controller` consumer boundary 现在都已经落地，Debian review-prep 文档也已经冻结文案契约、验收 recipe、review-packet template、bootstrap proof 指南 `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`、steady-state 指南 `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`、backup-restore 指南 `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md`、diagnostics 指南 `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md`、rollback 指南 `docs/operations/portmanager-debian-12-rollback-proof-capture.md`、live follow-up 指南 `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md` 与 operator ownership，而且有边界的 candidate host 注册/probe/bootstrap review-prep lane 也已经真实存在，所以剩余缺口收窄为用 scaffold -> assemble -> validate helper 工作流为下一份真实 live packet 做准备，再通过同一份 `live-transport-follow-up-summary.json` 契约替换默认公开真相，同时继续把支持声明锁住。',
    ...
    },
    {
      ...
      items: {
        en: [
          ...
          'Use the active requirements/plan pair as the current implementation map: `docs/brainstorms/2026-04-21-portmanager-m3-live-packet-execution-tooling-requirements.md` and `docs/plans/2026-04-21-portmanager-m3-live-packet-execution-tooling-plan.md`.',
          'Units 51 through 76 are now landed on that broader path; review-packet readiness still makes guide coverage and artifact coverage explicit, the earlier Units 63 through 69 queue now has one bounded review packet preserved at `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`, artifact coverage is now `20/20`, and `/second-target-policy-pack` now keeps data-driven `liveTransportFollowUp` truth: current default state stays `capture_required` through guide `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md`, while `capture_complete` only becomes real after one validator-passing committed packet keeps a valid `live-transport-follow-up-summary.json` plus all five live follow-up artifacts without scaffold markers.',
          'Move next through repo-native live packet execution tooling for declared candidate `debian-12-systemd-tailscale` under `/second-target-policy-pack`: use `pnpm milestone:scaffold:live-packet`, `pnpm milestone:assemble:live-packet`, then `pnpm milestone:validate:live-packet` while Docker bridge address `172.17.0.2` stays blocking truth instead of reopening already-landed seams or pretending standalone deployment pressure already exists.',
        ...
        zh: [
          ...
          '把当前激活的 requirements/plan 组合作为实现地图：`docs/brainstorms/2026-04-21-portmanager-m3-live-packet-execution-tooling-requirements.md` 与 `docs/plans/2026-04-21-portmanager-m3-live-packet-execution-tooling-plan.md`。',
          'Unit 51 到 Unit 76 现在都已落地；review-packet readiness 继续把 guide coverage 与 artifact coverage 分开公开，此前公开的 Units 63 through 69 队列现在也已经拥有一份有边界 review packet：`docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`，而 artifact coverage 现在是 `20/20`，`/second-target-policy-pack` 现在还会发布 data-driven `liveTransportFollowUp` 真相：默认状态仍然通过 `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md` 保持 `capture_required`，而 `capture_complete` 只会在一份通过 validator、移除了 scaffold marker 的已提交 packet 同时保留有效 `live-transport-follow-up-summary.json` 与全部五个 live follow-up 产物后才变成真实状态。',
          '下一步转向 repo-native live packet execution tooling：围绕已声明候选 `debian-12-systemd-tailscale` 继续处理 Docker bridge 地址 `172.17.0.2` 这个阻塞真相，先执行 `pnpm milestone:scaffold:live-packet`、再执行 `pnpm milestone:assemble:live-packet` 写入真实证据、再执行 `pnpm milestone:validate:live-packet`，而不是重新打开已经落地的 seam，或假装独立部署压力已经真实出现。',
...
export const roadmapMilestones: RoadmapMilestone[] = [
  ...
  },
  {
    ...
    developerFocus: {
      en: [
        'Use `docs/brainstorms/2026-04-21-portmanager-m3-live-packet-execution-tooling-requirements.md` and `docs/plans/2026-04-21-portmanager-m3-live-packet-execution-tooling-plan.md` as the active implementation map; Units 51 through 76 are landed on that path, review-packet readiness still makes guide coverage plus artifact coverage explicit, remote `main` CI parity is repaired after the stale live-loader `hold` expectation moved to `review_required`, and `/second-target-policy-pack` now also publishes `review_open`, pending adjudication verdicts, plus `liveTransportFollowUp.state: capture_required` on the preserved bounded review packet at `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`.',
        'Move next through repo-native live packet execution tooling for declared candidate `debian-12-systemd-tailscale` under `/second-target-policy-pack`: start with `pnpm milestone:scaffold:live-packet`, continue with `pnpm milestone:assemble:live-packet`, and run `pnpm milestone:validate:live-packet` while Docker bridge address `172.17.0.2` still stays blocking truth instead of reopening routing split, replay ownership, or already-landed target-profile and decision-pack seams.',
      ...
      zh: [
        '把 `docs/brainstorms/2026-04-21-portmanager-m3-live-packet-execution-tooling-requirements.md` 与 `docs/plans/2026-04-21-portmanager-m3-live-packet-execution-tooling-plan.md` 当作当前实现地图；其中 Unit 51 到 Unit 76 已经全部落地，review-packet readiness 已把 guide coverage 与 artifact coverage 显式公开，remote `main` CI 也已经在把 stale live-loader `hold` 预期修正到 `review_required` 后恢复一致，而且 `/second-target-policy-pack` 现在还会针对 `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/` 这份有边界 review packet 公开 `review_open`、待裁定 verdict 与 `liveTransportFollowUp.state: capture_required`。',
        '下一步转向 repo-native live packet execution tooling：围绕已声明候选 `debian-12-systemd-tailscale` 继续处理 Docker bridge 地址 `172.17.0.2` 这条阻塞真相，先执行 `pnpm milestone:scaffold:live-packet`、再执行 `pnpm milestone:assemble:live-packet` 写入真实证据、再执行 `pnpm milestone:validate:live-packet`，而不是重新把 controller transport 堆回去、重做 routing split、重做 replay owner，或重复补已经落地的 target-profile 与 decision pack seam。',
