...
export const roadmapDeveloperProgress = {
  ...
  lede: {
    en: 'This page now exposes the accepted Milestone 1 public slice, the mainline gate that keeps it honest, the shipped heartbeat/version, GitHub-backup, and remote-backup-replay slices, the canonical confidence routine, the durable confidence history bundle with run-trace metadata, and the remaining work to accumulate repeated green history on that same live agent-backed flow.',
    zh: '这个页面现在直接暴露已经完成验收的 Milestone 1 公共切片、持续保护主线的 gate、已经落地的 heartbeat/version、GitHub-backup 与 remote-backup-replay 切片、规范 confidence routine、带运行追踪元数据的持久 confidence history bundle，以及建立在同一条 live agent 切片之上的后续持续转绿历史积累工作。'
  },
  buckets: [
    {
      id: 'verified',
    ...
    },
    {
      ...
      items: {
        en: [
          ...
          'The remaining architecture gap is no longer proof orchestration or missing history scaffolding; the canonical routine and its persisted history bundle now exist and the remaining work is sustained green history on the same live slice.',
          'Mainline evidence collection now keeps `pnpm acceptance:verify` on PRs while `push main`, `workflow_dispatch`, and the daily schedule run `pnpm milestone:verify:confidence`, restore/save the confidence history bundle, and upload the bundle artifact.'
        ],
        zh: [
          ...
          '剩余架构缺口已经不再是证明编排，也不再是历史脚手架缺失；规范 confidence routine 与持久 history bundle 已存在，剩余工作已经收窄为同一条 live 切片上的持续转绿历史。',
          'mainline evidence collection 现在会继续把 `pnpm acceptance:verify` 保留在 PR 路径上，并在 `push main`、`workflow_dispatch` 与每日 schedule 路径上运行 `pnpm milestone:verify:confidence`、恢复并保存 confidence history bundle，再上传 bundle artifact。'
        ]
      }
    },
    {
      ...
      items: {
        en: [
          ...
          'Review uploaded confidence-history bundles and their run-trace metadata instead of relying on raw CI logs to judge green-history accumulation.',
        ...
        zh: [
          ...
          '优先查看上传后的 confidence-history bundle 及其运行追踪元数据，而不是继续只靠原始 CI 日志判断持续转绿历史。',
