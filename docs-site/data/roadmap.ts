...
export const roadmapDeveloperProgress = {
  ...
  lede: {
    en: 'This page now exposes three layers at once: the accepted Milestone 1 public slice, the still-mandatory Milestone 2 confidence guardrail, and the active Milestone 3 `Phase 0 enablement` lane. Current repo truth is no longer “Toward C someday” and no longer “Milestone 2 helper plumbing first.” Current repo truth is evidence-first progression: keep the same accepted live slice honest through review helpers and public counters, keep the Debian second-target docs contract, acceptance recipe, review-packet template, bootstrap-proof guide `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`, steady-state guide `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`, backup-restore guide `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md`, diagnostics guide `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md`, rollback guide `docs/operations/portmanager-debian-12-rollback-proof-capture.md`, live follow-up guide `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md`, and ownership artifacts aligned, keep the bounded candidate-host create/probe/bootstrap lane honest, then drive the preserved Debian 12 packet through `/second-target-policy-pack.liveTransportFollowUp` while broader support still stays locked to Ubuntu. Exact live counters stay on this development-progress page and the tracked confidence artifact.',
    zh: '这个页面现在同时暴露三层真实状态：已经完成验收的 Milestone 1 公共切片、仍然必须保持的 Milestone 2 confidence guardrail，以及已经进入执行中的 Milestone 3 `Phase 0 enablement` 主线。当前仓库真相已经不再是“Toward C 以后再说”，也不再是“先补 Milestone 2 helper 脚手架”；当前真相是 evidence-first 递进：继续用 review helper 与公开计数保护同一条 accepted live slice，保持 Debian 第二目标文档契约、验收 recipe、review-packet template、bootstrap proof 指南 `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`、steady-state 指南 `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`、backup-restore 指南 `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md`、diagnostics 指南 `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md`、rollback 指南 `docs/operations/portmanager-debian-12-rollback-proof-capture.md`、live follow-up 指南 `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md` 与 ownership 产物对齐，保持有边界的 candidate host 注册/probe/bootstrap lane 诚实，再通过 `/second-target-policy-pack.liveTransportFollowUp` 去推进已保留的 Debian 12 packet，同时继续把更广支持声明锁在 Ubuntu 上。精确实时计数继续留在这个 development-progress 页面与被跟踪 confidence artifact 上。'
  },
  buckets: [
    {
      id: 'verified',
      ...
      items: {
        en: [
          ...
          'The active implementation map now lives in `docs/brainstorms/2026-04-21-portmanager-m3-live-tailscale-follow-up-requirements.md` and `docs/plans/2026-04-21-portmanager-m3-live-tailscale-follow-up-plan.md`: guide coverage is complete, Units 63 through 71 are landed, `/second-target-policy-pack` now reports `review_open`, blocking delta `container_bridge_transport_substitution`, and top-level `liveTransportFollowUp.state: capture_required`, the preserved Docker bridge address is still `172.17.0.2`, guide `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md` names the next packet root, and the next bounded queue is live Tailscale follow-up while the locked target-profile registry, consumer-boundary decision pack, deployment-boundary decision pack, second-target policy pack, persistence decision pack, and Debian review-prep governance docs stay the truthful platform contract.',
        ...
        zh: [
          ...
          '当前激活的实现地图已经切到 `docs/brainstorms/2026-04-21-portmanager-m3-live-tailscale-follow-up-requirements.md` 与 `docs/plans/2026-04-21-portmanager-m3-live-tailscale-follow-up-plan.md`：guide coverage 已完整、Unit 63 到 Unit 71 已落地，`/second-target-policy-pack` 现在还会公开 `review_open`、阻塞 delta `container_bridge_transport_substitution` 与顶层 `liveTransportFollowUp.state: capture_required`，当前保留的 Docker bridge 地址仍然是 `172.17.0.2`，而 `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md` 已经把下一份 packet 根目录固定出来，同时继续让锁定 target-profile registry、consumer-boundary decision pack、deployment-boundary decision pack、second-target policy pack、persistence decision pack 与 Debian review-prep 治理文档保持为稳定平台契约。',
    ...
    },
    {
      ...
      items: {
        ...
        zh: [
          ...
          '索引化 event/audit review、显式 audit-review owner、target-profile registry 发布、persistence-readiness reporting、新的 `/persistence-decision-pack`、`/consumer-boundary-decision-pack` 与 `/deployment-boundary-decision-pack` 复核表面，以及兼容旧路由的 `/api/controller` consumer boundary 现在都已经落地，Debian review-prep 文档也已经冻结文案契约、验收 recipe、review-packet template、bootstrap proof 指南 `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`、steady-state 指南 `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`、backup-restore 指南 `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md`、diagnostics 指南 `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md`、rollback 指南 `docs/operations/portmanager-debian-12-rollback-proof-capture.md`、live follow-up 指南 `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md` 与 operator ownership，而且有边界的 candidate host 注册/probe/bootstrap review-prep lane 也已经真实存在，所以剩余缺口收窄为围绕 `/second-target-policy-pack` 把当前 Docker bridge 地址 `172.17.0.2` 替换为 live Tailscale-backed bounded evidence，同时继续把支持声明锁住。',
    ...
    },
    {
      ...
      items: {
        en: [
          ...
          'Use the active requirements/plan pair as the current implementation map: `docs/brainstorms/2026-04-21-portmanager-m3-live-tailscale-follow-up-requirements.md` and `docs/plans/2026-04-21-portmanager-m3-live-tailscale-follow-up-plan.md`.',
          'Units 51 through 71 are now landed on that map; review-packet readiness still makes guide coverage and artifact coverage explicit, the earlier Units 63 through 69 queue now has one bounded review packet preserved at `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`, artifact coverage is now `20/20`, and `/second-target-policy-pack` now keeps data-driven `liveTransportFollowUp` truth: current default state stays `capture_required` through guide `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md`, while `capture_complete` only becomes real after one fresh packet root plus all five live follow-up artifacts are recorded.',
          'Move next through live Tailscale follow-up for declared candidate `debian-12-systemd-tailscale` under `/second-target-policy-pack`, with Docker bridge address `172.17.0.2` still treated as blocking truth instead of reopening already-landed seams or pretending standalone deployment pressure already exists.',
        ...
        zh: [
          ...
          '把当前激活的 requirements/plan 组合作为实现地图：`docs/brainstorms/2026-04-21-portmanager-m3-live-tailscale-follow-up-requirements.md` 与 `docs/plans/2026-04-21-portmanager-m3-live-tailscale-follow-up-plan.md`。',
          'Unit 51 到 Unit 71 现在都已落地；review-packet readiness 继续把 guide coverage 与 artifact coverage 分开公开，此前公开的 Units 63 through 69 队列现在也已经拥有一份有边界 review packet：`docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`，而 artifact coverage 现在是 `20/20`，`/second-target-policy-pack` 现在还会发布 data-driven `liveTransportFollowUp` 真相：默认状态仍然通过 `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md` 保持 `capture_required`，而 `capture_complete` 只会在一份新的 packet 根目录与全部五个 live follow-up 产物都被记录后才变成真实状态。',
          '下一步转向 live Tailscale follow-up：围绕已声明候选 `debian-12-systemd-tailscale` 继续处理 Docker bridge 地址 `172.17.0.2` 这个阻塞真相，而不是重新打开已经落地的 seam，或假装独立部署压力已经真实出现。',
...
export const roadmapMilestones: RoadmapMilestone[] = [
  ...
  },
  {
    ...
    developerFocus: {
      en: [
        'Use `docs/brainstorms/2026-04-21-portmanager-m3-live-tailscale-follow-up-requirements.md` and `docs/plans/2026-04-21-portmanager-m3-live-tailscale-follow-up-plan.md` as the active implementation map; Units 51 through 71 are landed on that path, review-packet readiness still makes guide coverage plus artifact coverage explicit, and `/second-target-policy-pack` now also publishes `review_open`, pending adjudication verdicts, plus `liveTransportFollowUp.state: capture_required` on the preserved bounded review packet at `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`.',
        'Move next through live Tailscale follow-up for declared candidate `debian-12-systemd-tailscale` under `/second-target-policy-pack`, with Docker bridge address `172.17.0.2` still treated as blocking truth instead of reopening routing split, replay ownership, or already-landed target-profile and decision-pack seams.',
      ...
      zh: [
        '把 `docs/brainstorms/2026-04-21-portmanager-m3-live-tailscale-follow-up-requirements.md` 与 `docs/plans/2026-04-21-portmanager-m3-live-tailscale-follow-up-plan.md` 当作当前实现地图；其中 Unit 51 到 Unit 71 已经全部落地，review-packet readiness 已把 guide coverage 与 artifact coverage 显式公开，而且 `/second-target-policy-pack` 现在还会针对 `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/` 这份有边界 review packet 公开 `review_open`、待裁定 verdict 与 `liveTransportFollowUp.state: capture_required`。',
        '下一步转向 live Tailscale follow-up：围绕已声明候选 `debian-12-systemd-tailscale` 继续处理 Docker bridge 地址 `172.17.0.2` 这条阻塞真相，而不是重新把 controller transport 堆回去、重做 routing split、重做 replay owner，或重复补已经落地的 target-profile 与 decision pack seam。',
