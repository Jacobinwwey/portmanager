    ...
    },
    {
      ...
      items: {
        ...
        zh: [
          ...
          '索引化 event/audit review、显式 audit-review owner、target-profile registry 发布、persistence-readiness reporting、新的 `/persistence-decision-pack`、`/consumer-boundary-decision-pack` 与 `/deployment-boundary-decision-pack` 复核表面，以及兼容旧路由的 `/api/controller` consumer boundary 现在都已经落地，Debian review-prep 文档也已经冻结文案契约、验收 recipe、review-packet template、bootstrap proof 指南 `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`、steady-state 指南 `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`、backup-restore 指南 `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md`、diagnostics 指南 `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md`、rollback 指南 `docs/operations/portmanager-debian-12-rollback-proof-capture.md`、live follow-up 指南 `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md` 与 operator ownership，而且有边界的 candidate host 注册/probe/bootstrap review-prep lane 也已经真实存在，所以剩余缺口收窄为用 scaffold 与 validator helper 为下一份真实 live packet 做准备，再通过同一份 `live-transport-follow-up-summary.json` 契约替换默认公开真相，同时继续把支持声明锁住。',
    ...
    },
    {
      ...
      items: {
        en: [
          ...
          'Move next through repo-native live packet execution tooling for declared candidate `debian-12-systemd-tailscale` under `/second-target-policy-pack`: use `pnpm milestone:scaffold:live-packet`, replace scaffold markers with real evidence, then run `pnpm milestone:validate:live-packet` while Docker bridge address `172.17.0.2` stays blocking truth instead of reopening already-landed seams or pretending standalone deployment pressure already exists.',
        ...
        zh: [
          ...
          '下一步转向 repo-native live packet execution tooling：围绕已声明候选 `debian-12-systemd-tailscale` 继续处理 Docker bridge 地址 `172.17.0.2` 这个阻塞真相，先执行 `pnpm milestone:scaffold:live-packet`、再替换真实证据、再执行 `pnpm milestone:validate:live-packet`，而不是重新打开已经落地的 seam，或假装独立部署压力已经真实出现。',
  ...
  },
  {
    ...
    developerFocus: {
      ...
      zh: [
        ...
        '下一步转向 repo-native live packet execution tooling：围绕已声明候选 `debian-12-systemd-tailscale` 继续处理 Docker bridge 地址 `172.17.0.2` 这条阻塞真相，先执行 `pnpm milestone:scaffold:live-packet`、再替换真实证据、再执行 `pnpm milestone:validate:live-packet`，而不是重新把 controller transport 堆回去、重做 routing split、重做 replay owner，或重复补已经落地的 target-profile 与 decision pack seam。',
