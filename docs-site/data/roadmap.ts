  ...
  buckets: [
    {
      ...
      items: {
        en: [
          ...
          'The active implementation map now lives in `docs/brainstorms/2026-04-21-portmanager-m3-review-delta-surface-requirements.md` and `docs/plans/2026-04-21-portmanager-m3-review-delta-surface-plan.md`: guide coverage is complete, Units 63 through 76 are landed history, `/second-target-policy-pack` still reports `review_open`, pending adjudication verdicts, top-level `liveTransportFollowUp.state: capture_required`, and explicit `reviewAdjudication.blockingDeltas`, remote `main` CI parity is repaired after the stale live-loader `hold` expectation moved to `review_required`, blocking delta `container_bridge_transport_substitution` keeps preserved Docker bridge address `172.17.0.2` visible as public truth, and the only bounded follow-up is one real live packet through `pnpm milestone:scaffold:live-packet`, `pnpm milestone:assemble:live-packet`, `pnpm milestone:validate:live-packet`, and the same `live-transport-follow-up-summary.json` contract while the locked target-profile registry, consumer-boundary decision pack, deployment-boundary decision pack, second-target policy pack, persistence decision pack, and Debian review-prep governance docs stay the truthful platform contract.',
        ...
        zh: [
          ...
          '当前激活的实现地图已经切到 `docs/brainstorms/2026-04-21-portmanager-m3-review-delta-surface-requirements.md` 与 `docs/plans/2026-04-21-portmanager-m3-review-delta-surface-plan.md`：guide coverage 已完整、Units 63 到 76 已成为已落地历史，`/second-target-policy-pack` 现在还会公开 `review_open`、待裁定 verdict、顶层 `liveTransportFollowUp.state: capture_required` 与显式 `reviewAdjudication.blockingDeltas`，remote `main` CI 也已经在把 stale live-loader `hold` 预期修正到 `review_required` 后恢复一致，阻塞 delta `container_bridge_transport_substitution` 会继续把保留中的 Docker bridge 地址 `172.17.0.2` 公开为当前真相，而当前唯一有边界 follow-up 已经收窄为通过 `pnpm milestone:scaffold:live-packet`、`pnpm milestone:assemble:live-packet`、`pnpm milestone:validate:live-packet` 与同一份 `live-transport-follow-up-summary.json` 契约写入一份真实 live packet，同时继续让锁定 target-profile registry、consumer-boundary decision pack、deployment-boundary decision pack、second-target policy pack、persistence decision pack 与 Debian review-prep 治理文档保持为稳定平台契约。',
    ...
    },
    {
      ...
      items: {
        ...
        zh: [
          ...
          '把当前激活的 requirements/plan 组合作为实现地图：`docs/brainstorms/2026-04-21-portmanager-m3-review-delta-surface-requirements.md` 与 `docs/plans/2026-04-21-portmanager-m3-review-delta-surface-plan.md`。',
          ...
          '下一步转向有边界的 live Tailscale follow-up：围绕已声明候选 `debian-12-systemd-tailscale` 继续处理 `container_bridge_transport_substitution` 与 Docker bridge 地址 `172.17.0.2` 这条阻塞真相，先执行 `pnpm milestone:scaffold:live-packet`、再执行 `pnpm milestone:assemble:live-packet` 写入真实证据、再执行 `pnpm milestone:validate:live-packet`，而不是重新打开已经落地的 seam，或假装独立部署压力已经真实出现。',
...
export const roadmapMilestones: RoadmapMilestone[] = [
  ...
  },
  {
    ...
    developerFocus: {
      ...
      zh: [
        '把 `docs/brainstorms/2026-04-21-portmanager-m3-review-delta-surface-requirements.md` 与 `docs/plans/2026-04-21-portmanager-m3-review-delta-surface-plan.md` 当作当前实现地图；其中 Unit 51 到 Unit 76 已经全部落地，review-packet readiness 已把 guide coverage 与 artifact coverage 显式公开，remote `main` CI 也已经在把 stale live-loader `hold` 预期修正到 `review_required` 后恢复一致，而且 `/second-target-policy-pack` 现在还会针对 `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/` 这份有边界 review packet 公开 `review_open`、待裁定 verdict、`reviewAdjudication.blockingDeltas` 与 `liveTransportFollowUp.state: capture_required`。',
        '下一步转向有边界的 live Tailscale follow-up：围绕已声明候选 `debian-12-systemd-tailscale` 继续处理阻塞 delta `container_bridge_transport_substitution` 与 Docker bridge 地址 `172.17.0.2` 这条阻塞真相，先执行 `pnpm milestone:scaffold:live-packet`、再执行 `pnpm milestone:assemble:live-packet` 写入真实证据、再执行 `pnpm milestone:validate:live-packet`，而不是重新把 controller transport 堆回去、重做 routing split、重做 replay owner，或重复补已经落地的 target-profile 与 decision pack seam。',
