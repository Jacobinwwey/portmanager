...
export const roadmapDeveloperProgress = {
  ...
  lede: {
    en: 'This page now exposes the accepted Milestone 1 public slice, the mainline gate that keeps it honest, the shipped heartbeat/version, GitHub-backup, and remote-backup-replay slices, the canonical confidence routine, the durable confidence report artifact with run-trace metadata, and the remaining work to accumulate repeated green history on that same live agent-backed flow.',
    zh: '这个页面现在直接暴露已经完成验收的 Milestone 1 公共切片、持续保护主线的 gate、已经落地的 heartbeat/version、GitHub-backup 与 remote-backup-replay 切片、规范 confidence routine、带运行追踪元数据的持久 confidence 报告 artifact，以及建立在同一条 live agent 切片之上的后续持续转绿历史积累工作。'
  },
  buckets: [
    {
      id: 'verified',
      ...
      items: {
        en: [
          ...
          '`pnpm milestone:verify:confidence` now writes `.portmanager/reports/milestone-confidence-report.json` with `eventName` / `ref` / `sha` / `runId` / `runAttempt` / `workflow` traceability metadata, and CI uploads that same report as a developer-facing artifact.',
        ...
        zh: [
          ...
          '`pnpm milestone:verify:confidence` 现在还会写出带 `eventName` / `ref` / `sha` / `runId` / `runAttempt` / `workflow` traceability 元数据的 `.portmanager/reports/milestone-confidence-report.json`，CI 也会把同一份报告上传成面向开发者的 artifact。',
    ...
    },
    {
      ...
      items: {
        en: [
          ...
          'Mainline evidence collection now keeps `pnpm acceptance:verify` on PRs while `push main`, `workflow_dispatch`, and the daily schedule run `pnpm milestone:verify:confidence` and upload the confidence report artifact.'
        ],
        zh: [
          ...
          'mainline evidence collection 现在会继续把 `pnpm acceptance:verify` 保留在 PR 路径上，并在 `push main`、`workflow_dispatch` 与每日 schedule 路径上运行 `pnpm milestone:verify:confidence` 并上传 confidence 报告 artifact。'
        ]
      }
    },
    {
      ...
      items: {
        en: [
          ...
          'Keep `pnpm milestone:verify:confidence` green on `main`, manual runs, and the daily scheduled history lane.',
          'Review uploaded confidence-report artifacts and their run-trace metadata instead of relying on raw CI logs to judge green-history accumulation.',
        ...
        zh: [
          ...
          '继续让 `pnpm milestone:verify:confidence` 在 `main`、手动触发与每日 schedule 历史路径上持续转绿。',
          '优先查看上传后的 confidence-report artifact 及其运行追踪元数据，而不是继续只靠原始 CI 日志判断持续转绿历史。',
