...
export const roadmapDeveloperProgress = {
  ...
  lede: {
    en: 'This page now exposes three layers at once: the accepted Milestone 1 public slice, the still-mandatory Milestone 2 confidence guardrail, and the active Milestone 3 `Phase 0 enablement` lane. Current repo truth is no longer “Toward C someday” and no longer “Milestone 2 helper plumbing first.” Current repo truth is evidence-first progression: keep the same accepted live slice honest through review helpers and public counters, keep the new Debian review-prep docs contract, acceptance recipe, and ownership artifacts aligned, then extend the already landed Units 51 through 62 from declared candidate `debian-12-systemd-tailscale` toward transport parity and recovery parity under `/second-target-policy-pack` while keeping the deployment-boundary and persistence decision packs stable. Exact live counters stay on this development-progress page and the tracked confidence artifact.',
    zh: '这个页面现在同时暴露三层真实状态：已经完成验收的 Milestone 1 公共切片、仍然必须保持的 Milestone 2 confidence guardrail，以及已经进入执行中的 Milestone 3 `Phase 0 enablement` 主线。当前仓库真相已经不再是“Toward C 以后再说”，也不再是“先补 Milestone 2 helper 脚手架”；当前真相是 evidence-first 递进：继续用 review helper 与公开计数保护同一条 accepted live slice，保持新的 Debian review-prep 文档契约、验收 recipe 与 ownership 产物对齐，再把已经落地的 Unit 51 到 Unit 62 从已声明的 `debian-12-systemd-tailscale` 候选推进到围绕 `/second-target-policy-pack` 的传输等价与恢复等价，同时保持 deployment-boundary 与 persistence decision pack 稳定。精确实时计数继续留在这个 development-progress 页面与被跟踪 confidence artifact 上。'
  },
  buckets: [
    {
      id: 'verified',
    ...
    },
    {
      ...
      items: {
        en: [
          ...
          'Indexed event/audit review, explicit audit-review ownership, target-profile registry publication, persistence-readiness reporting, the new `/persistence-decision-pack`, `/consumer-boundary-decision-pack`, `/deployment-boundary-decision-pack`, and `/second-target-policy-pack` review surfaces, plus the compatibility-safe `/api/controller` consumer boundary are now all landed, and Debian review-prep docs now freeze contract wording, acceptance recipe, and operator ownership. The remaining gap therefore narrows to transport parity and recovery parity under `/second-target-policy-pack`, plus any later standalone artifact proof only when pressure becomes real.',
        ...
        zh: [
          ...
          '索引化 event/audit review、显式 audit-review owner、target-profile registry 发布、persistence-readiness reporting、新的 `/persistence-decision-pack`、`/consumer-boundary-decision-pack` 与 `/deployment-boundary-decision-pack` 复核表面，以及兼容旧路由的 `/api/controller` consumer boundary 现在都已经落地，Debian review-prep 文档也已经冻结文案契约、验收 recipe 与 operator ownership，所以剩余缺口收窄为 `/second-target-policy-pack` 下的传输等价与恢复等价，以及只在真实压力出现时才需要的独立部署产物证明。',
    ...
    },
    {
      ...
      items: {
        en: [
          ...
          'Move next toward transport parity and recovery parity proof for declared candidate `debian-12-systemd-tailscale` under `/second-target-policy-pack` instead of reopening already-landed seams or pretending standalone deployment pressure already exists.',
        ...
        zh: [
          ...
          '下一步转向围绕已声明候选 `debian-12-systemd-tailscale` 的传输等价与恢复等价证明，而不是重新打开已经落地的 seam，或假装独立部署压力已经真实出现。',
  ...
  },
  {
    ...
    developerFocus: {
      en: [
        ...
        'Move next to transport parity and recovery parity proof for declared candidate `debian-12-systemd-tailscale` under `/second-target-policy-pack` instead of reopening routing split, replay ownership, or already-landed target-profile and decision-pack seams.',
      ...
      zh: [
        ...
        '下一步转向已声明候选 `debian-12-systemd-tailscale` 的传输/恢复/文档/验收/owner 证明，而不是重新把 controller transport 堆回去、重做 routing split、重做 replay owner，或重复补已经落地的 target-profile 与 decision pack seam。',
