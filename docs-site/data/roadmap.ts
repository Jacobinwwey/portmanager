...
export const roadmapDeveloperProgress = {
  ...
  lede: {
    en: 'This page now exposes three layers at once: the accepted Milestone 1 public slice, the still-mandatory Milestone 2 confidence guardrail, and the active Milestone 3 `Phase 0 enablement` lane. Current repo truth is no longer “Toward C someday” and no longer “Milestone 2 helper plumbing first.” Current repo truth is evidence-first progression: keep the same accepted live slice honest through review helpers and public counters, keep the new Debian review-prep docs contract, acceptance recipe, review-packet template, bootstrap-proof guide `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`, steady-state guide `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`, backup-restore guide `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md`, diagnostics guide `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md`, rollback guide `docs/operations/portmanager-debian-12-rollback-proof-capture.md`, and ownership artifacts aligned, keep the newly landed candidate-host create/probe/bootstrap review-prep lane honest, then push from declared candidate `debian-12-systemd-tailscale` toward review-packet capture through those five guides while keeping rollback parity blocked under `/second-target-policy-pack` until real evidence exists. Exact live counters stay on this development-progress page and the tracked confidence artifact.',
    zh: '这个页面现在同时暴露三层真实状态：已经完成验收的 Milestone 1 公共切片、仍然必须保持的 Milestone 2 confidence guardrail，以及已经进入执行中的 Milestone 3 `Phase 0 enablement` 主线。当前仓库真相已经不再是“Toward C 以后再说”，也不再是“先补 Milestone 2 helper 脚手架”；当前真相是 evidence-first 递进：继续用 review helper 与公开计数保护同一条 accepted live slice，保持新的 Debian review-prep 文档契约、验收 recipe、review-packet template、bootstrap proof 指南 `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`、steady-state 指南 `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`、backup-restore 指南 `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md`、diagnostics 指南 `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md`、rollback 指南 `docs/operations/portmanager-debian-12-rollback-proof-capture.md` 与 ownership 产物对齐，保持新落地的 candidate host 注册/probe/bootstrap review-prep lane 诚实，再把已声明的 `debian-12-systemd-tailscale` 候选推进到围绕 `/second-target-policy-pack` 的 review packet 落盘与五份采集指南执行，同时继续把 rollback parity 锁成阻塞项，直到真实证据出现。精确实时计数继续留在这个 development-progress 页面与被跟踪 confidence artifact 上。'
  },
  buckets: [
    {
      id: 'verified',
    ...
    },
    {
      ...
      items: {
        ...
        zh: [
          ...
          '索引化 event/audit review、显式 audit-review owner、target-profile registry 发布、persistence-readiness reporting、新的 `/persistence-decision-pack`、`/consumer-boundary-decision-pack` 与 `/deployment-boundary-decision-pack` 复核表面，以及兼容旧路由的 `/api/controller` consumer boundary 现在都已经落地，Debian review-prep 文档也已经冻结文案契约、验收 recipe、review-packet template、bootstrap proof 指南 `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`、steady-state 指南 `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`、backup-restore 指南 `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md`、diagnostics 指南 `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md`、rollback 指南 `docs/operations/portmanager-debian-12-rollback-proof-capture.md` 与 operator ownership，而且有边界的 candidate host 注册/probe/bootstrap review-prep lane 也已经真实存在，所以剩余缺口收窄为 `/second-target-policy-pack` 下依次执行五份 proof guide，再推进 rollback 等价，以及只在真实压力出现时才需要的独立部署产物证明。',
    ...
    },
    {
      ...
      items: {
        en: [
          ...
          'Units 51 through 62 are now landed on that map, including the new consumer-boundary split criteria, deployment-boundary decision pack, persistence promotion decision pack, and second-target policy pack.',
          'Move next toward keeping the candidate-host create/probe/bootstrap review-prep lane honest, then landing the review-packet template path, bootstrap-proof guide `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`, steady-state guide `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`, backup-restore guide `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md`, diagnostics guide `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md`, rollback guide `docs/operations/portmanager-debian-12-rollback-proof-capture.md`, and the remaining rollback parity for declared candidate `debian-12-systemd-tailscale` under `/second-target-policy-pack` instead of reopening already-landed seams or pretending standalone deployment pressure already exists.',
        ...
        zh: [
          ...
          'Unit 51 到 Unit 62 现在都已落地，其中已经包含新的 consumer-boundary split criteria、deployment-boundary decision pack、persistence promotion decision pack 与 second-target policy pack。',
          '下一步转向先把已声明候选 `debian-12-systemd-tailscale` 的 candidate host 注册/probe/bootstrap review-prep lane 保持诚实，再推进 review packet 模板落盘、bootstrap proof 指南 `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`、steady-state 指南 `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`、backup-restore 指南 `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md`、diagnostics 指南 `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md` 与剩余 rollback 等价证明，而不是重新打开已经落地的 seam，或假装独立部署压力已经真实出现。',
...
export const roadmapMilestones: RoadmapMilestone[] = [
  ...
  },
  {
    ...
    developerFocus: {
      en: [
        'Use `docs/brainstorms/2026-04-21-portmanager-m3-toward-c-enablement-requirements.md` and `docs/plans/2026-04-21-portmanager-m3-toward-c-enablement-plan.md` as the active implementation map; Units 51 through 62 are landed on that path.',
        'Move next to keeping the candidate-host create/probe/bootstrap review-prep lane honest, then landing the review-packet template path, bootstrap-proof guide `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`, steady-state guide `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`, backup-restore guide `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md`, diagnostics guide `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md`, rollback guide `docs/operations/portmanager-debian-12-rollback-proof-capture.md`, and the remaining rollback parity for declared candidate `debian-12-systemd-tailscale` under `/second-target-policy-pack` instead of reopening routing split, replay ownership, or already-landed target-profile and decision-pack seams.',
      ...
      zh: [
        '把 `docs/brainstorms/2026-04-21-portmanager-m3-toward-c-enablement-requirements.md` 与 `docs/plans/2026-04-21-portmanager-m3-toward-c-enablement-plan.md` 当作当前实现地图；其中 Unit 51 到 Unit 62 已经全部落地。',
        '下一步转向先把已声明候选 `debian-12-systemd-tailscale` 的 candidate host 注册/probe/bootstrap review-prep lane 保持诚实，再推进 review packet 模板落盘、bootstrap proof 指南 `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`、steady-state 指南 `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`、backup-restore 指南 `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md`、diagnostics 指南 `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md` 与剩余 rollback/文档/验收/owner 证明，而不是重新把 controller transport 堆回去、重做 routing split、重做 replay owner，或重复补已经落地的 target-profile 与 decision pack seam。',
