  ...
  buckets: [
    {
      ...
      items: {
        en: [
          ...
          'Milestone 3 Units 51 through 62 are now real: controller seam extraction baseline, bounded batch exposure-policy envelope, explicit `/event-audit-index`, `/persistence-readiness`, `/persistence-decision-pack`, `/consumer-boundary-decision-pack`, `/deployment-boundary-decision-pack`, and `/second-target-policy-pack` generated contracts, shared CLI/Web review parity, a SQLite-backed persistence adapter that measures PostgreSQL readiness from live store counts, a compatibility-safe `/api/controller` consumer boundary, one explicit `audit-review-service` owner behind replay plus indexed review, one explicit locked target-profile registry published across controller, CLI, and Web, one consumer-boundary split-criteria pack that keeps `/api/controller` embedded until standalone conditions are real, one deployment-boundary decision pack that keeps standalone review on hold until deployability evidence is real, one second-target policy pack that keeps support locked to Ubuntu while declaring `debian-12-systemd-tailscale` as a review-prep candidate, and one migration-review surface that keeps SQLite active while making cutover pressure explicit.',
          'The active plan now shifts the concrete next queue for developers beyond Unit 62 toward bootstrap transport, steady-state transport, backup and restore, diagnostics, rollback, docs, acceptance, and ownership proof for `debian-12-systemd-tailscale` under `/second-target-policy-pack` while the locked target-profile registry, consumer-boundary decision pack, deployment-boundary decision pack, second-target policy pack, and persistence decision pack stay the truthful platform contract.',
    ...
    },
    {
      ...
      items: {
        en: [
          ...
          'Indexed event/audit review, explicit audit-review ownership, target-profile registry publication, persistence-readiness reporting, the new `/persistence-decision-pack`, `/consumer-boundary-decision-pack`, `/deployment-boundary-decision-pack`, and `/second-target-policy-pack` review surfaces, plus the compatibility-safe `/api/controller` consumer boundary are now all landed, so the remaining gap narrows from declared candidate `debian-12-systemd-tailscale` to transport parity, recovery parity, docs, acceptance, and ownership proof under `/second-target-policy-pack`, plus any later standalone artifact proof only when pressure becomes real.',
        ...
        zh: [
          ...
          '索引化 event/audit review、显式 audit-review owner、target-profile registry 发布、persistence-readiness reporting、新的 `/persistence-decision-pack`、`/consumer-boundary-decision-pack` 与 `/deployment-boundary-decision-pack` 复核表面，以及兼容旧路由的 `/api/controller` consumer boundary 现在都已经落地，所以剩余缺口收窄为更聚焦的第二目标策略，以及只在真实压力出现时才需要的独立部署产物证明。',
    ...
    },
    {
      ...
      items: {
        en: [
          ...
          'Move next toward transport parity, recovery parity, docs, acceptance, and ownership proof for declared candidate `debian-12-systemd-tailscale` under `/second-target-policy-pack` instead of reopening already-landed seams or pretending standalone deployment pressure already exists.',
        ...
        zh: [
          ...
          '下一步转向围绕已声明候选 `debian-12-systemd-tailscale` 的传输等价、恢复等价、文档、验收与 ownership 证明，而不是重新打开已经落地的 seam，或假装独立部署压力已经真实出现。',
  ...
  },
  {
    ...
    developerFocus: {
      en: [
        ...
        'Move next to transport parity, recovery parity, docs, acceptance, and ownership proof for declared candidate `debian-12-systemd-tailscale` under `/second-target-policy-pack` instead of reopening routing split, replay ownership, or already-landed target-profile and decision-pack seams.',
