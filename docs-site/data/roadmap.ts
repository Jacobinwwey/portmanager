  ...
  buckets: [
    {
      ...
      items: {
        en: [
          ...
          'Milestone 3 Units 51 through 62 are now real: controller seam extraction baseline, bounded batch exposure-policy envelope, explicit `/event-audit-index`, `/persistence-readiness`, `/persistence-decision-pack`, `/consumer-boundary-decision-pack`, `/deployment-boundary-decision-pack`, and `/second-target-policy-pack` generated contracts, shared CLI/Web review parity, a SQLite-backed persistence adapter that measures PostgreSQL readiness from live store counts, a compatibility-safe `/api/controller` consumer boundary, one explicit `audit-review-service` owner behind replay plus indexed review, one explicit locked target-profile registry published across controller, CLI, and Web, one consumer-boundary split-criteria pack that keeps `/api/controller` embedded until standalone conditions are real, one deployment-boundary decision pack that keeps standalone review on hold until deployability evidence is real, one second-target policy pack that keeps support locked to Ubuntu until candidate-target parity evidence is real, and one migration-review surface that keeps SQLite active while making cutover pressure explicit.',
          'The active plan now shifts the concrete next queue for developers beyond Unit 62 toward candidate-target declaration, parity evidence, docs, acceptance, and ownership proof under `/second-target-policy-pack` while the locked target-profile registry, consumer-boundary decision pack, deployment-boundary decision pack, second-target policy pack, and persistence decision pack stay the truthful platform contract.',
        ...
        zh: [
          ...
          'Milestone 3 的 Unit 51 到 Unit 62 现在都已真实落地：controller seam extraction baseline、bounded batch exposure-policy envelope、显式 `/event-audit-index`、`/persistence-readiness`、`/persistence-decision-pack`、`/consumer-boundary-decision-pack`、`/deployment-boundary-decision-pack` 与 `/second-target-policy-pack` 生成契约、共享 CLI/Web 复核一致性、会从真实 store 计数衡量 PostgreSQL readiness 的 SQLite-backed persistence adapter、兼容旧路由的 `/api/controller` consumer boundary、统一接管 replay 与 indexed review 查询的 `audit-review-service` owner、已经在 controller、CLI、Web 发布的锁定 target-profile registry、明确 `/api/controller` 继续内嵌条件的 consumer-boundary split-criteria pack、明确独立部署复核仍然不能开启条件的 deployment-boundary decision pack、继续把支持声明锁在 Ubuntu 画像内的 second-target policy pack，以及继续让 SQLite 保持 active 的 migration-review surface 都已经建立，而且没有改动已验收 evidence model。',
          '当前实现地图也已经把开发者的下一批具体工作转向 Unit 62 之后围绕 `/second-target-policy-pack` 的候选第二目标声明、等价证据、文档、验收与 ownership 证明，同时继续把锁定 target-profile registry、consumer-boundary decision pack、deployment-boundary decision pack、second-target policy pack 与 persistence decision pack 保持为稳定平台契约。',
    ...
    },
    {
      ...
      items: {
        en: [
          ...
          'Move next toward candidate-target declaration, parity evidence, docs, acceptance, and ownership proof under `/second-target-policy-pack` instead of reopening already-landed seams or pretending standalone deployment pressure already exists.',
        ...
        zh: [
          ...
          '下一步转向围绕 `/second-target-policy-pack` 的候选第二目标声明、等价证据、文档、验收与 ownership 证明，而不是重新打开已经落地的 seam，或假装独立部署压力已经真实出现。',
  ...
  },
  {
    ...
    developerFocus: {
      en: [
        ...
        'Move next to candidate-target declaration, parity evidence, docs, acceptance, and ownership proof under `/second-target-policy-pack` instead of reopening routing split, replay ownership, or already-landed target-profile and decision-pack seams.',
      ...
      zh: [
        ...
        '下一步转向更聚焦的第二目标策略，而不是重新把 controller transport 堆回去、重做 routing split、重做 replay owner，或重复补已经落地的 target-profile 与 decision pack seam。',
