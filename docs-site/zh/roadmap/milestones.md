...
| --- | --- | --- |
| gateway-ready 的 consumer boundary | Web 与 CLI 仍然通过 `REST + SSE` 直接访问 controller，但 bounded batch envelope 与 `/event-audit-index` 已经开始把契约面收敛成更 gateway-ready 的形状 | Phase 0 baseline 已落地 |
