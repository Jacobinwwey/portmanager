...
| --- | --- | --- |
| gateway-ready 的 consumer 边界 | Web 与 CLI 仍然通过 `REST + SSE` 直接访问 controller，但 `/event-audit-index` 与 `/persistence-readiness` 已经发布出更清晰的 review/readiness 契约面 | Phase 0 baseline 已落地 |
