...
| --- | --- | --- | --- |
| Consumer gateway boundary | No dedicated gateway app or service yet, but controller now serves a gateway-ready `/api/controller` consumer boundary while keeping legacy direct routes compatible, `/consumer-boundary-decision-pack` states why routing stays embedded, `/deployment-boundary-decision-pack` states why standalone deployment review still stays on hold, and `/second-target-policy-pack` now also publishes review-packet readiness so `debian-12-systemd-tailscale` stays review-prep with guide coverage complete, artifact coverage at `8/20`, and one preserved bootstrap-plus-steady-state review packet at `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/` | Phase 0 baseline landed | Next focus shifts to Units 66 through 69 on top of the landed readiness surface: backup-and-restore, diagnostics, rollback, and second-target review closeout under `/second-target-policy-pack`, not more routing-shell churn |
...
| Second-target platform abstraction | Ubuntu 24.04 + systemd + Tailscale remains the only supported target, while `debian-12-systemd-tailscale` now stays explicitly bounded inside `/second-target-policy-pack` with review-packet readiness, guide coverage, artifact coverage `8/20`, one preserved bootstrap-plus-steady-state review packet, and the earlier Units 63 through 69 queue narrowed to Units 66 through 69 | Phase 0 baseline landed | Execute Units 66 through 69 before any broader second-target claim or abstraction rewrite |
...
| --- | --- | --- | --- |
| Consumer gateway boundary | 当前还没有独立 gateway app 或 service，但 controller 已经开始提供 gateway-ready 的 `/api/controller` consumer boundary，并继续兼容旧直连路由；`/consumer-boundary-decision-pack` 已明确说明这条边界为什么现在仍应保持内嵌，`/deployment-boundary-decision-pack` 也已明确说明为什么独立部署复核仍应保持在 hold，而 `/second-target-policy-pack` 现在还会公开 review-packet readiness，明确 `debian-12-systemd-tailscale` 仍然只是 review-prep，guide coverage 已完整、artifact coverage 来到 `8/20`，并且已经保留一份 `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/` bootstrap 加 steady-state review packet | Phase 0 baseline 已落地 | 下一焦点已经切到建立在 readiness surface 之上的 Unit 66 到 Unit 69：backup-and-restore、diagnostics、rollback 与 second-target review closeout，而不是继续做路由外壳工作 |
...
| 第二目标画像的平台抽象 | Ubuntu 24.04 + systemd + Tailscale 仍是唯一支持目标，而 `debian-12-systemd-tailscale` 现在已经在 `/second-target-policy-pack` 里通过 review-packet readiness、guide coverage、artifact coverage `8/20`、已保留的 bootstrap 加 steady-state review packet，以及从此前 Units 63 through 69 收窄到 Unit 66 到 Unit 69 的队列，被明确限制在有边界 review-prep 姿态里 | Phase 0 baseline 已落地 | 先执行 Unit 66 到 Unit 69，再讨论任何更广的第二目标声明或 abstraction rewrite |
