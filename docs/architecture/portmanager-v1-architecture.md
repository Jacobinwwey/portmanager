...
| --- | --- | --- | --- |
| Consumer gateway boundary | No dedicated gateway app or service yet, but controller now serves a gateway-ready `/api/controller` consumer boundary while keeping legacy direct routes compatible, `/consumer-boundary-decision-pack` states why routing stays embedded, `/deployment-boundary-decision-pack` states why standalone deployment review still stays on hold, and `/second-target-policy-pack` now also publishes review-packet readiness so `debian-12-systemd-tailscale` stays bounded with guide coverage complete, artifact coverage at `20/20`, one preserved bootstrap-plus-steady-state-plus-backup-plus-diagnostics-plus-rollback review packet at `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`, and `decisionState: review_required` while broader support remains locked | Phase 0 baseline landed | Next focus shifts from packet execution to bounded second-target review adjudication under `/second-target-policy-pack`, not more routing-shell churn |
...
| Second-target platform abstraction | Ubuntu 24.04 + systemd + Tailscale remains the only supported target, while `debian-12-systemd-tailscale` now stays explicitly bounded inside `/second-target-policy-pack` with review-packet readiness, guide coverage, artifact coverage `20/20`, one preserved bootstrap-plus-steady-state-plus-backup-plus-diagnostics-plus-rollback review packet, and `decisionState: review_required` instead of support expansion | Phase 0 baseline landed | Finish bounded second-target review before any broader second-target claim or abstraction rewrite |
...
| 第二目标画像的平台抽象 | Ubuntu 24.04 + systemd + Tailscale 仍是唯一支持目标，而 `debian-12-systemd-tailscale` 现在已经在 `/second-target-policy-pack` 里通过 review-packet readiness、guide coverage、artifact coverage `20/20`、已保留的 bootstrap、steady-state、backup、diagnostics 与 rollback review packet，以及 `decisionState: review_required`，被明确限制在有边界 review 姿态里，而不是自动变成已支持目标 | Phase 0 baseline 已落地 | 先完成 bounded second-target review，再讨论任何更广的第二目标声明或 abstraction rewrite |
