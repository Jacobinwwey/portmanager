...
| --- | --- | --- | --- |
| Consumer gateway boundary | 当前还没有独立 gateway app 或 service，但 controller 已经开始提供 gateway-ready 的 `/api/controller` consumer boundary，并继续兼容旧直连路由；`/consumer-boundary-decision-pack` 已明确说明这条边界为什么现在仍应保持内嵌，`/deployment-boundary-decision-pack` 也已明确说明为什么独立部署复核仍应保持在 hold，而 `/second-target-policy-pack` 现在还会公开 review-packet readiness、`reviewAdjudication.state: review_open` 与阻塞 delta `container_bridge_transport_substitution`，明确 `debian-12-systemd-tailscale` 现在已经拥有 guide coverage 完整、artifact coverage `20/20` 的完整 review packet，但当前 Docker bridge 地址 `172.17.0.2` 仍然替代 live Tailscale transport，所以更广支持声明继续锁定 | Phase 0 baseline 已落地 | 下一焦点已经从 packet 执行切到 `/second-target-policy-pack` 下围绕 Docker bridge transport-substitution 阻塞项的 live Tailscale follow-up，而不是继续做路由外壳工作 |
