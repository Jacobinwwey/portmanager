...
| --- | --- | --- | --- |
| Consumer gateway boundary | No dedicated gateway app or service yet, but controller now serves a gateway-ready `/api/controller` consumer boundary while keeping legacy direct routes compatible, `/consumer-boundary-decision-pack` states why routing stays embedded, `/deployment-boundary-decision-pack` states why standalone deployment review still stays on hold, and `/second-target-policy-pack` now also publishes review-packet readiness, `reviewAdjudication.state: review_open`, blocking delta `container_bridge_transport_substitution`, plus `liveTransportFollowUp.state: capture_required` with guide path `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md` so `debian-12-systemd-tailscale` stays bounded with guide coverage complete, artifact coverage at `20/20`, one preserved bootstrap-plus-steady-state-plus-backup-plus-diagnostics-plus-rollback review packet at `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`, and Docker bridge address `172.17.0.2` still replacing live Tailscale transport while broader support remains locked | Phase 0 baseline landed | Next focus shifts from packet execution to one live Tailscale follow-up for the explicit Docker-bridge transport-substitution blocker under `/second-target-policy-pack`, not more routing-shell churn |
...
| Second-target platform abstraction | Ubuntu 24.04 + systemd + Tailscale remains the only supported target, while `debian-12-systemd-tailscale` now stays explicitly bounded inside `/second-target-policy-pack` with review-packet readiness, `reviewAdjudication.state: review_open`, blocking delta `container_bridge_transport_substitution`, `liveTransportFollowUp.state: capture_required`, guide coverage, artifact coverage `20/20`, one preserved bootstrap-plus-steady-state-plus-backup-plus-diagnostics-plus-rollback review packet, and Docker bridge address `172.17.0.2` still standing in for live Tailscale transport instead of support expansion | Phase 0 baseline landed | Finish the live-Tailscale follow-up for that blocking delta before any broader second-target claim or abstraction rewrite |
...
| --- | --- | --- | --- |
| Consumer gateway boundary | 当前还没有独立 gateway app 或 service，但 controller 已经开始提供 gateway-ready 的 `/api/controller` consumer boundary，并继续兼容旧直连路由；`/consumer-boundary-decision-pack` 已明确说明这条边界为什么现在仍应保持内嵌，`/deployment-boundary-decision-pack` 也已明确说明为什么独立部署复核仍应保持在 hold，而 `/second-target-policy-pack` 现在还会公开 review-packet readiness、`reviewAdjudication.state: review_open`、阻塞 delta `container_bridge_transport_substitution` 与 `liveTransportFollowUp.state: capture_required`，并把 guide path 固定到 `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md`，明确 `debian-12-systemd-tailscale` 现在已经拥有 guide coverage 完整、artifact coverage `20/20` 的完整 review packet，但当前 Docker bridge 地址 `172.17.0.2` 仍然替代 live Tailscale transport，所以更广支持声明继续锁定 | Phase 0 baseline 已落地 | 下一焦点已经从 packet 执行切到 `/second-target-policy-pack` 下围绕 Docker bridge transport-substitution 阻塞项的 live Tailscale follow-up，而不是继续做路由外壳工作 |
...
| 第二目标画像的平台抽象 | Ubuntu 24.04 + systemd + Tailscale 仍是唯一支持目标，而 `debian-12-systemd-tailscale` 现在已经在 `/second-target-policy-pack` 里通过 review-packet readiness、`reviewAdjudication.state: review_open`、阻塞 delta `container_bridge_transport_substitution`、`liveTransportFollowUp.state: capture_required`、guide coverage、artifact coverage `20/20`、已保留的 bootstrap、steady-state、backup、diagnostics 与 rollback review packet，以及当前仍在替代 live Tailscale transport 的 Docker bridge 地址 `172.17.0.2`，被明确限制在有边界 review 姿态里，而不是自动变成已支持目标 | Phase 0 baseline 已落地 | 先完成这条阻塞 delta 的 live Tailscale follow-up，再讨论任何更广的第二目标声明或 abstraction rewrite |
