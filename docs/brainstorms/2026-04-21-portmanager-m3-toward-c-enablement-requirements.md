...
Status note on `2026-04-21`: deep comparison now shows Milestone 1 accepted, Milestone 2 promotion-ready thresholds met, and Units 51 through 56 already landed. PortManager is no longer only entering Milestone 3; it is now inside Milestone 3 `Phase 0 enablement` with consumer-boundary, audit-index, and persistence-readiness baselines in code. PortManager still has not earned full Scheme C claims because standalone audit/event boundaries, explicit target-abstraction rules, and a real PostgreSQL backend path remain unfinished.
...
- the first Milestone 3 seams are now real: `/api/controller`, `controller-domain-service`, `controller-read-model`, `/event-audit-index`, `/persistence-readiness`, and the SQLite-backed persistence adapter all landed without changing the accepted evidence model
...
| Platform breadth grows through explicit abstractions | Supported target still remains `Ubuntu 24.04 + systemd + Tailscale`; no explicit target profile registry exists yet | Not started | Milestone 3 should define target-abstraction rules before second-target claims appear |
...
- R4. Milestone 3 continuation must define bounded workstreams for standalone audit/event boundary decisions, consumer-boundary split criteria, persistence migration decision criteria, and target-profile abstraction rules on top of the already landed Units 51 through 56 baselines.
...
- R8. `README.md`, `TODO.md`, `Interface Document.md`, `docs/specs/portmanager-milestones.md`, `docs/specs/portmanager-v1-product-spec.md`, `docs/specs/portmanager-toward-c-strategy.md`, and `docs/architecture/portmanager-v1-architecture.md` must all describe the same Milestone 3 truth: Units 51 through 56 are landed, but the repo still lacks the full Scheme C runtime shape.
- R9. Roadmap and development-progress pages must surface the refreshed requirements/plan pair, show landed Units 51 through 56 plus queued Units 57 through 59 for developers, explain the current architecture gap map, and keep the Milestone 2 review commands visible.
...
## Success Criteria
- Contributors can explain why Milestone 3 started now, what Units 51 through 56 already delivered, why the phase is still bounded, and what concrete architecture work comes next.
...
- Root docs, milestone docs, strategy docs, and architecture docs all agree on the same posture: Milestone 2 remains a guardrail, Units 51 through 56 are landed baselines, and Units 57 through 59 are the next bounded continuation lane.
...
- Treat Units 51 through 56 as landed baselines and move the next implementation map to Units 57 through 59 rather than restarting earlier seam work.
...
- Existing `/api/controller`, `/event-audit-index`, and `/persistence-readiness` routes remain the real starting point for the next continuation units.

## Next Steps
- Move to `docs/plans/2026-04-21-portmanager-m3-toward-c-enablement-plan.md` for the Phase 0 continuation sequence covering Units 57 through 59.
