...
- The preserved packet root is `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`, and current controller truth is guide coverage `6/6`, artifact coverage `20/20`, `decisionState: review_required`, `reviewAdjudication.state: review_open`, and `liveTransportFollowUp.state: capture_required`.
...
- The current follow-up guide is `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md`, and the fresh packet root pattern is `docs/operations/artifacts/debian-12-live-tailscale-packet-<date>/`.
...
- Keep docs, CLI, Web, controller contract, and roadmap pages aligned with the same `review_required`, `review_open`, `blockingDeltas`, and `liveTransportFollowUp` posture.
...
- Follow-up guide: `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md`
- Follow-up artifact root pattern: `docs/operations/artifacts/debian-12-live-tailscale-packet-<date>/`
- Required follow-up: capture one live Tailscale-backed bounded packet for `debian-12-systemd-tailscale` by following the dedicated guide, or keep support locked to Ubuntu with the delta still published
- Landed governance artifacts today: docs contract, acceptance recipe, review-packet template, operator ownership definition, live-Tailscale follow-up guide, and review-delta contract data
...
- 当前保留 packet 根目录是 `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`，controller 真相已经来到 guide coverage `6/6`、artifact coverage `20/20`、`decisionState: review_required`、`reviewAdjudication.state: review_open` 与 `liveTransportFollowUp.state: capture_required`。
...
- 当前 follow-up guide 是 `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md`，而新的 packet 根目录模式是 `docs/operations/artifacts/debian-12-live-tailscale-packet-<date>/`。
...
- docs、CLI、Web、controller contract 与 roadmap 页面必须保持同一份 `review_required`、`review_open`、`blockingDeltas` 与 `liveTransportFollowUp` 真相。
...
- follow-up guide：`docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md`
- follow-up artifact root pattern：`docs/operations/artifacts/debian-12-live-tailscale-packet-<date>/`
- 当前必需 follow-up：按照专门的 follow-up guide，为 `debian-12-systemd-tailscale` 捕获一份 live Tailscale-backed bounded packet，否则继续把支持声明锁在 Ubuntu
- 当前已落地治理产物：docs contract、acceptance recipe、review-packet template、operator ownership definition、live-Tailscale follow-up guide，以及 review delta 合同数据
