...
Updated: 2026-04-18
Version: v0.3.2-docs-site-confidence-publication
...
### Confidence progress publication contract
- `docs-site/data/milestone-confidence-progress.ts` is a tracked publication artifact, not a machine-local cache.
- Default docs commands must reuse the committed artifact:
  - `pnpm --dir docs-site --ignore-workspace run docs:generate`
  - `pnpm --dir docs-site --ignore-workspace run docs:build`
- Only one command may refresh that tracked artifact from local `.portmanager` history:
  - `pnpm --dir docs-site --ignore-workspace run docs:generate:refresh-confidence`
- The explicit refresh command should be used only after deliberate verification or sync activity whose evidence should become the new published snapshot.
- This boundary keeps normal docs builds hermetic and prevents machine-local confidence history from creating noisy tracked diffs.
...
### Confidence progress 发布契约
- `docs-site/data/milestone-confidence-progress.ts` 是一个被跟踪的发布产物，而不是 machine-local 缓存。
- 默认 docs 命令必须复用已提交产物：
  - `pnpm --dir docs-site --ignore-workspace run docs:generate`
  - `pnpm --dir docs-site --ignore-workspace run docs:build`
- 只有一条命令允许把本地 `.portmanager` history 刷新进这个被跟踪产物：
  - `pnpm --dir docs-site --ignore-workspace run docs:generate:refresh-confidence`
- 这条显式刷新命令只能在一次有意的 verification 或 sync 之后使用，用于把那次证据提升为新的公开快照。
- 这条边界的目的，是让普通 docs build 继续保持 hermetic，并避免 machine-local confidence history 制造持续噪音 diff。
