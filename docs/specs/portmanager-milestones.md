...
- `Next lane`: Milestone 3 now opens as bounded `Phase 0 enablement` on top of the same live host / rule / policy slice. Milestone 2 review helpers remain mandatory guardrails: keep `pnpm milestone:review:promotion-ready -- --limit 20` as the default completed-mainline review entrypoint, use `pnpm milestone:fetch:review-pack` when the current CI run is the first question, keep the public development-progress page plus `.portmanager/reports/milestone-wording-review.md` as the wording-truth bundle, and only move the tracked public snapshot through the same helper plus `--refresh-published-artifact` when review agrees. Units 57 through 62 are now landed, including the explicit `/consumer-boundary-decision-pack`, `/deployment-boundary-decision-pack`, `/persistence-decision-pack`, and `/second-target-policy-pack` review surfaces, and the Debian review-prep governance slice now freezes docs contract, acceptance recipe, review-packet template, bootstrap-proof guide `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`, steady-state guide `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`, backup-restore guide `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md`, diagnostics guide `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md`, rollback guide `docs/operations/portmanager-debian-12-rollback-proof-capture.md`, and ownership sources. The bounded candidate-host create/probe/bootstrap review-prep lane is also now real. Bootstrap parity, steady-state parity, backup parity, diagnostics parity, and rollback parity all remain review-prep only, so new implementation energy now shifts from declared candidate `debian-12-systemd-tailscale` toward review-packet capture through those five guides plus the remaining rollback parity while the locked target-profile registry, consumer-boundary decision pack, deployment-boundary decision pack, second-target policy pack, and persistence decision pack remain the truthful platform contract.
...
- Next queue: keep the landed consumer boundary, consumer-boundary decision pack, deployment-boundary decision pack, second-target policy pack, audit-review boundary, target-profile registry, persistence decision pack, and Debian review-prep governance docs stable while keeping the candidate-host create/probe/bootstrap review-prep lane honest and proving review-packet capture, bootstrap proof capture through `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`, steady-state proof capture through `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`, backup-restore proof capture through `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md`, diagnostics proof capture through `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md`, rollback proof capture through `docs/operations/portmanager-debian-12-rollback-proof-capture.md`, plus the remaining rollback parity for declared candidate `debian-12-systemd-tailscale` under `/second-target-policy-pack`
...
- `下一主线`：Milestone 3 现在已经作为有边界的 `Phase 0 enablement` 打开，但仍然建立在同一条 live host / rule / policy 切片上。Milestone 2 的 review helper 继续保留为 guardrail：把 `pnpm milestone:review:promotion-ready -- --limit 20` 作为 completed-mainline 之后的默认复核入口；如果第一问题是当前 CI run，就先执行 `pnpm milestone:fetch:review-pack` 并读取 `.portmanager/reports/current-ci-review-pack/`；继续把公开 development-progress 页面、`.portmanager/reports/milestone-wording-review.md` 与验证报告当作文案真相包；只在人工复核同意时通过同一条 helper 加上 `--refresh-published-artifact` 推进公开快照。Unit 57 到 Unit 62 现在都已落地，其中已经包含显式 `/consumer-boundary-decision-pack`、`/deployment-boundary-decision-pack`、`/persistence-decision-pack` 与 `/second-target-policy-pack` 四类复核表面，而且 Debian review-prep 治理切片也已经冻结文档契约、验收 recipe、review-packet template、bootstrap proof 指南 `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md`、steady-state 指南 `docs/operations/portmanager-debian-12-steady-state-proof-capture.md`、backup-restore 指南 `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md`、diagnostics 指南 `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md`、rollback 指南 `docs/operations/portmanager-debian-12-rollback-proof-capture.md` 与 ownership 来源；现在 candidate host 注册、probe、bootstrap review-prep lane 也已经落地，但 bootstrap parity、steady-state parity、backup parity、diagnostics parity 与 rollback parity 仍未完成证明，所以新的实现主线现在转向 review packet 落盘、五份证明采集指南执行与其余 rollback 等价，同时继续把锁定 target-profile registry、consumer-boundary decision pack、deployment-boundary decision pack、second-target policy pack 与 persistence decision pack 保持为真实平台契约。
...
- 下一批队列：继续把已落地的 consumer boundary、consumer-boundary decision pack、deployment-boundary decision pack、second-target policy pack、audit-review boundary、target-profile registry 与 persistence decision pack 保持稳定，同时把 candidate host 注册、probe、bootstrap review-prep lane 保持诚实，再证明围绕已声明候选 `debian-12-systemd-tailscale` 的 review packet 落盘、通过 `docs/operations/portmanager-debian-12-bootstrap-proof-capture.md` 采集 bootstrap 证明、通过 `docs/operations/portmanager-debian-12-steady-state-proof-capture.md` 采集 steady-state 证明、通过 `docs/operations/portmanager-debian-12-backup-restore-proof-capture.md` 采集 backup-restore 证明、通过 `docs/operations/portmanager-debian-12-diagnostics-proof-capture.md` 采集 diagnostics 证明、其余 rollback、文档/验收/owner 证据
