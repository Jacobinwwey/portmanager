...
| --- | --- | --- |
| Gateway-ready consumer boundary | Web and CLI still call the controller directly over `REST + SSE`, but the bounded batch envelope plus explicit `/event-audit-index` and `/persistence-readiness` contracts now shape one cleaner gateway-ready review/readiness surface | Phase 0 baseline landed |
...
| --- | --- | --- |
| gateway-ready 的 consumer boundary | Web 与 CLI 仍然通过 `REST + SSE` 直接访问 controller，但 bounded batch envelope 加上显式 `/event-audit-index` 与 `/persistence-readiness` 契约，已经开始把 review/readiness 契约面收敛成更 gateway-ready 的形状 | Phase 0 baseline 已落地 |
