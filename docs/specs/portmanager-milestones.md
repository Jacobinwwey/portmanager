...
- `pmg` is the primary CLI prefix for milestone-facing operator and agent workflows; compatibility aliases must not dilute that default in docs or acceptance guidance.
- Review/evidence work remains a guardrail for publication truth and high-risk changes, not the default pacing mechanism for normal implementation slices.
...
- `Next lane`: Milestone 3 now opens as bounded `Phase 0 enablement` on top of the same live host / rule / policy slice. Milestone 2 review helpers remain mandatory publication and high-risk guardrails rather than default implementation blockers: use `pnpm milestone:review:promotion-ready -- --limit 20` after completed confidence runs or before public-claim changes, use `pnpm milestone:fetch:review-pack` when the current CI run is the first question, keep the public development-progress page plus `.portmanager/reports/milestone-wording-review.md` as the wording-truth bundle, and only move the tracked public snapshot through the same helper plus `--refresh-published-artifact` when review agrees. Units 57 through 62 remain landed enablement seams, Units 63 through 79 now stay landed history, and the active Milestone 3 map is now `docs/brainstorms/2026-04-22-portmanager-m3-live-packet-capture-preflight-requirements.md` plus `docs/plans/2026-04-22-portmanager-m3-live-packet-capture-preflight-plan.md` while the earlier source-auto-resolution pair stays landed history, `/second-target-policy-pack.reviewAdjudication.blockingDeltas` keeps delta `container_bridge_transport_substitution` and Docker bridge truth `172.17.0.2` explicit, `/second-target-policy-pack.liveTransportFollowUp` keeps `capture_required`, points at `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md`, and now waits on one real committed live packet with `pnpm milestone:preview:live-packet -- --packet-date <date> --controller-base-url <url>` as the preferred bounded read-only preflight, `pnpm milestone:capture:live-packet -- --packet-date <date> --controller-base-url <url>` as the bounded collector after preview reports ready, while `--candidate-target-profile-id`, `--host-id`, and `--bootstrap-operation-id` stay bounded overrides and scaffold, assemble, plus validator follow-up stay the fallback and verification layers. The Debian review-prep governance slice still freezes docs contract, acceptance recipe, review-packet template, five proof guides, the live follow-up guide, and ownership sources; remote `main` CI parity is repaired after the stale live-loader `hold` expectation moved to `review_required`; the candidate-host create/probe/bootstrap lane plus one bounded live steady-state mutation, one bounded backup operation, one diagnostics run, and one rollback rehearsal stay bounded; and broader target support remains blocked while review remains open.
...
- Verified now: `pnpm milestone:verify:confidence` now composes the standing `pnpm acceptance:verify` gate with the remote-backup replay proof, and `.github/workflows/mainline-acceptance.yml` now keeps that heavier routine on explicit `workflow_dispatch` plus the daily scheduled history run while ordinary `push main` stays on the lighter acceptance path.
...
- Verified now: the persisted confidence history now classifies `local-only`, `building-history`, and `promotion-ready`, measures progress against `7` qualified runs plus `3` consecutive qualified passes from the qualified history scope (`push`, `workflow_dispatch`, and `schedule` on `refs/heads/main`), and publishes the same summary in the workflow run page for developers.
...
- `pmg` 是面向里程碑的 operator 与 agent 工作流主 CLI 前缀；兼容别名可以存在，但不能稀释文档与验收指导中的这个默认入口。
- review/evidence 工作继续作为公开文案真相与高风险变更的护栏，而不是普通实现切片的默认节奏控制器。
...
- `下一主线`：Milestone 3 现在已经作为有边界的 `Phase 0 enablement` 打开，但仍然建立在同一条 live host / rule / policy 切片上。Milestone 2 的 review helper 继续保留为发布真相与高风险变更的 guardrail，而不是普通实现切片的默认阻塞器：在完成 confidence run 后或准备修改公开 claim 时执行 `pnpm milestone:review:promotion-ready -- --limit 20`；如果第一问题是当前 CI run，就先执行 `pnpm milestone:fetch:review-pack` 并读取 `.portmanager/reports/current-ci-review-pack/`；继续把公开 development-progress 页面、`.portmanager/reports/milestone-wording-review.md` 与验证报告当作文案真相包；只在人工复核同意时通过同一条 helper 加上 `--refresh-published-artifact` 推进公开快照。Unit 57 到 Unit 62 继续作为已落地 seam 保持稳定，Units 63 到 79 现在都已经成为已落地历史，而第一份完整有边界 Debian 12 review packet 继续保留到 `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`：guide coverage 已完整、artifact coverage 保持 `20/20`、bootstrap、steady-state、backup-and-restore、diagnostics 与 rollback parity 已落地，remote `main` CI 也已经在把 stale live-loader `hold` 预期修正到 `review_required` 后恢复一致。当前激活的 Milestone 3 地图已经切到 `docs/brainstorms/2026-04-22-portmanager-m3-live-packet-capture-preflight-requirements.md` 与 `docs/plans/2026-04-22-portmanager-m3-live-packet-capture-preflight-plan.md`，而此前的 source auto-resolution pair 继续作为已落地历史存在，同时 `/second-target-policy-pack.reviewAdjudication.blockingDeltas` 会继续把阻塞 delta `container_bridge_transport_substitution` 与 Docker bridge 真相 `172.17.0.2` 显式公开，`/second-target-policy-pack.liveTransportFollowUp` 会继续保持 `capture_required`、指向 `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md`，并继续把新的 packet 根目录模式固定为 `docs/operations/artifacts/debian-12-live-tailscale-packet-<date>/`；当前真实缺口已经收敛为继续显式公开这个阻塞 delta，先把 `pnpm milestone:preview:live-packet -- --packet-date <date> --controller-base-url <url>` 作为首选只读预检，再把 `pnpm milestone:capture:live-packet -- --packet-date <date> --controller-base-url <url>` 作为首选采集路径，把 `--candidate-target-profile-id`、`--host-id` 与 `--bootstrap-operation-id` 保留为有边界 override，再把 scaffold、assemble 与 validator helper 保留为回退与核验层。Debian review-prep 治理切片继续冻结文档契约、验收 recipe、review-packet template、五份 proof 指南、live follow-up guide 与 ownership 来源；candidate host 注册、probe、bootstrap review-prep lane 继续保持有边界，并额外允许一条有边界 live steady-state mutation、一条有边界 backup operation、一条 diagnostics run 与一条 rollback rehearsal；同时继续把锁定 target-profile registry、consumer-boundary decision pack、deployment-boundary decision pack、second-target policy pack 与 persistence decision pack 保持为真实平台契约。
...
- 当前已验证：`pnpm milestone:verify:confidence` 现在已经把既有 `pnpm acceptance:verify` gate 与 remote-backup replay proof 收敛成一条规范 routine，`.github/workflows/mainline-acceptance.yml` 现在会把这条更重的 routine 收敛到显式 `workflow_dispatch` 与每日 schedule 历史路径上，而普通 `push main` 继续保持较轻的 acceptance 路径。
...
- 当前已验证：持久 confidence history 现在会区分 `local-only`、`building-history`、`promotion-ready` 三种 readiness 状态，按照 qualified history scope（`refs/heads/main` 上的 `push`、`workflow_dispatch`、`schedule`）的 `7` 次 qualified run 加 `3` 次连续 qualified pass 统计推进进度，并把同一份 summary 直接发布到 workflow 页面给开发者查看。
