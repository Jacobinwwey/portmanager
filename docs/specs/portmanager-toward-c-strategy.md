...
- Closeout slice on top: `docs/brainstorms/2026-04-27-portmanager-m2-closeout-incus-runway-requirements.md` plus `docs/plans/2026-04-27-portmanager-m2-closeout-incus-runway-plan.md` now freeze the repo-native `pnpm milestone:rehearse:debian12-incus -- --name portmanager-debian12-review` helper and the first frontend runway surfaces so Milestone 2 guardrail discipline plus this bounded Toward C queue stay visible together.
...
- 下一队列：先把 candidate host 注册、probe、bootstrap review-prep lane 保持诚实，再遵循 `docs/brainstorms/2026-04-22-portmanager-m3-live-packet-capture-preflight-requirements.md` 与 `docs/plans/2026-04-22-portmanager-m3-live-packet-capture-preflight-plan.md`，并把此前的 source auto-resolution pair 视为已落地历史：Units 63 到 79 现在都已成为已落地历史，第一份完整有边界 Debian 12 review packet 继续保留到 `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`，artifact coverage 维持 `20/20`，`/second-target-policy-pack` 现在已经把 `reviewAdjudication.blockingDeltas` 与 `liveTransportFollowUp.state: capture_required` 通过 `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md` 显式公开，remote `main` CI 也已经在把 stale live-loader `hold` 预期修正到 `review_required` 后恢复一致，而当前实时队列已经收敛为继续处理阻塞 delta `container_bridge_transport_substitution`、替换 Docker bridge 真相 `172.17.0.2`，先把 `pnpm milestone:preview:live-packet -- --packet-date <date> --controller-base-url <url>` 作为首选 repo-native 只读预检，再把 `pnpm milestone:capture:live-packet -- --packet-date <date> --controller-base-url <url>` 作为写入路径，把 `--candidate-target-profile-id`、`--host-id` 与 `--bootstrap-operation-id` 保留为有边界 override，把 scaffold -> assemble -> validate helper 保留为回退与核验层，而不是 PostgreSQL 默认状态库承诺或伪造的独立部署推进
- 收尾切片：`docs/brainstorms/2026-04-27-portmanager-m2-closeout-incus-runway-requirements.md` 与 `docs/plans/2026-04-27-portmanager-m2-closeout-incus-runway-plan.md` 现在继续把 repo-native `pnpm milestone:rehearse:debian12-incus -- --name portmanager-debian12-review` helper 与第一批前端 runway surface 落盘，让 Milestone 2 guardrail 与这条有边界 Toward C 队列同时保持可见。
