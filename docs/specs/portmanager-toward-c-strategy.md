...
- Next queue: keep the candidate-host create/probe/bootstrap review-prep lane honest, then follow `docs/brainstorms/2026-04-21-portmanager-m3-live-tailscale-follow-up-requirements.md` plus `docs/plans/2026-04-21-portmanager-m3-live-tailscale-follow-up-plan.md`: Units 63 through 71 stay landed history, the first complete bounded Debian 12 review packet remains preserved at `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`, artifact coverage stays `20/20`, `/second-target-policy-pack` now keeps `liveTransportFollowUp.state: capture_required` explicit through guide `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md`, and the live queue is now the Docker-bridge-to-live-Tailscale follow-up under the same locked support contract, not a PostgreSQL default-store claim or a fake standalone deployment push
...
- 下一队列：先把 candidate host 注册、probe、bootstrap review-prep lane 保持诚实，再遵循 `docs/brainstorms/2026-04-21-portmanager-m3-live-tailscale-follow-up-requirements.md` 与 `docs/plans/2026-04-21-portmanager-m3-live-tailscale-follow-up-plan.md`：Units 63 到 71 现在都已成为已落地历史，第一份完整有边界 Debian 12 review packet 继续保留到 `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`，artifact coverage 维持 `20/20`，`/second-target-policy-pack` 现在已经把 `liveTransportFollowUp.state: capture_required` 通过 `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md` 显式公开，而当前实时队列已经收敛为围绕 Docker bridge 地址 `172.17.0.2` 的 live Tailscale follow-up，而不是 PostgreSQL 默认状态库承诺或伪造的独立部署推进
