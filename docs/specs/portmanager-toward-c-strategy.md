...
- Next queue: keep the candidate-host create/probe/bootstrap review-prep lane honest, then follow `docs/brainstorms/2026-04-21-portmanager-m3-live-packet-source-auto-resolution-requirements.md` plus `docs/plans/2026-04-21-portmanager-m3-live-packet-source-auto-resolution-plan.md`: Units 63 through 79 stay landed history, the first complete bounded Debian 12 review packet remains preserved at `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`, artifact coverage stays `20/20`, `/second-target-policy-pack` now keeps `reviewAdjudication.blockingDeltas` and `liveTransportFollowUp.state: capture_required` explicit through guide `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md`, remote `main` CI parity is repaired after the stale live-loader `hold` expectation moved to `review_required`, and the live queue is now the bounded follow-up that keeps `container_bridge_transport_substitution` plus Docker bridge truth `172.17.0.2` visible while `pnpm milestone:capture:live-packet -- --packet-date <date> --controller-base-url <url>` becomes the preferred repo-native collector and explicit ids remain bounded overrides for one real packet under the same locked support contract, not a PostgreSQL default-store claim or a fake standalone deployment push
...
- 下一队列：先把 candidate host 注册、probe、bootstrap review-prep lane 保持诚实，再遵循 `docs/brainstorms/2026-04-21-portmanager-m3-live-packet-source-auto-resolution-requirements.md` 与 `docs/plans/2026-04-21-portmanager-m3-live-packet-source-auto-resolution-plan.md`：Units 63 到 79 现在都已成为已落地历史，第一份完整有边界 Debian 12 review packet 继续保留到 `docs/operations/artifacts/debian-12-bootstrap-packet-2026-04-21/`，artifact coverage 维持 `20/20`，`/second-target-policy-pack` 现在已经把 `reviewAdjudication.blockingDeltas` 与 `liveTransportFollowUp.state: capture_required` 通过 `docs/operations/portmanager-debian-12-live-tailscale-follow-up-capture.md` 显式公开，remote `main` CI 也已经在把 stale live-loader `hold` 预期修正到 `review_required` 后恢复一致，而当前实时队列已经收敛为继续处理阻塞 delta `container_bridge_transport_substitution`、替换 Docker bridge 真相 `172.17.0.2`，再把 `pnpm milestone:capture:live-packet -- --packet-date <date> --controller-base-url <url>` 作为首选 repo-native 采集路径，把 `--candidate-target-profile-id`、`--host-id` 与 `--bootstrap-operation-id` 保留为有边界 override，把 scaffold -> assemble -> validate helper 保留为回退与核验层，而不是 PostgreSQL 默认状态库承诺或伪造的独立部署推进
