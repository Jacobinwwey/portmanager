...
const currentReviewPackDir = '.portmanager/reports/current-ci-review-pack'
const currentReviewPackManifestName = 'review-pack-manifest.json'
const currentReviewPackManifestPath = path.join(
  repoRoot,
  '.portmanager',
  'reports',
  'current-ci-review-pack',
  currentReviewPackManifestName
)
...
const currentReviewPackHelperCommand = 'pnpm milestone:fetch:review-pack'
...
function normalizeReviewPackFiles(files = {}) {
  return Object.fromEntries(
    Object.keys(files)
      .sort((left, right) => left.localeCompare(right))
      .map((fileName) => [fileName, `${currentReviewPackDir}/${fileName}`])
  )
}

function reduceCurrentReviewPack(manifest) {
  if (!manifest) {
    return null
  }

  return {
    manifestVersion: manifest.manifestVersion ?? '0.1.0',
    fetchedAt: manifest.fetchedAt ?? null,
    repo: manifest.repo ?? null,
    workflowRef: manifest.workflowRef ?? null,
    branch: manifest.branch ?? null,
    artifactPattern: manifest.artifactPattern ?? null,
    outputDir: currentReviewPackDir,
    manifestPath: `${currentReviewPackDir}/${currentReviewPackManifestName}`,
    helperCommand: currentReviewPackHelperCommand,
    sourceRun: {
      id: manifest.sourceRun?.id ?? null,
      attempt: manifest.sourceRun?.attempt ?? null,
      event: manifest.sourceRun?.event ?? null,
      conclusion: manifest.sourceRun?.conclusion ?? null,
      status: manifest.sourceRun?.status ?? null,
      htmlUrl: manifest.sourceRun?.htmlUrl ?? null,
      workflowUrl: manifest.sourceRun?.workflowUrl ?? null,
      headSha: manifest.sourceRun?.headSha ?? null,
      createdAt: manifest.sourceRun?.createdAt ?? null,
      updatedAt: manifest.sourceRun?.updatedAt ?? null
    },
    files: {
      required: normalizeReviewPackFiles(manifest.files?.required),
      optional: normalizeReviewPackFiles(manifest.files?.optional)
    }
  }
}

async function loadCurrentReviewPack(reviewPackManifestSourcePath) {
  try {
    const manifestText = await fs.readFile(reviewPackManifestSourcePath, 'utf8')
    return reduceCurrentReviewPack(JSON.parse(manifestText))
  } catch (error) {
    if (error?.code === 'ENOENT') {
      return null
    }
...
function confidenceProgressSource(history, wordingReview, currentReviewPack) {
  return {
...
export async function generateMilestoneConfidenceProgressData({
  ...
  reviewPackManifestSourcePath = currentReviewPackManifestPath,
  ...
  try {
    ...
    const currentReviewPack = await loadCurrentReviewPack(reviewPackManifestSourcePath)
    const output = platformLineEndings([
      '/* This file is generated by scripts/docs/extract-locales.mjs. */',
      '',
      `export const milestoneConfidenceProgress = ${JSON.stringify(confidenceProgressSource(history, wordingReview, currentReviewPack), null, 2)} as const`,
      ''
    ].join('\n'))
