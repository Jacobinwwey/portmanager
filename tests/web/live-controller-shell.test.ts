...
test('web live loaders render controller-backed overview, host, rules, backups, and console surfaces', async () => {
  ...
  try {
    ...
    try {
      ...
      assert.equal(overviewState.deploymentBoundaryDecisionPack.decisionState, 'hold')
      assert.equal(overviewState.persistenceDecisionPack.readiness.status, 'healthy')
      assert.equal(overviewState.secondTargetPolicyPack.decisionState, 'review_required')
      assert.equal(overviewState.secondTargetPolicyPack.reviewAdjudication.state, 'review_open')
      assert.equal(overviewState.secondTargetPolicyPack.liveTransportFollowUp.state, 'capture_required')
      assert.equal(consoleState.selectedOperation?.id, diagnosticsAccepted.operationId)
      assert.equal(consoleState.deploymentBoundaryDecisionPack.decisionState, 'hold')
      assert.equal(consoleState.persistenceDecisionPack.readiness.status, 'healthy')
      assert.equal(consoleState.secondTargetPolicyPack.decisionState, 'review_required')
      assert.equal(consoleState.secondTargetPolicyPack.reviewAdjudication.state, 'review_open')
      assert.equal(consoleState.secondTargetPolicyPack.liveTransportFollowUp.state, 'capture_required')
      ...
      assert.match(overviewHtml, /Deployment boundary/i)
      assert.match(overviewHtml, /Persistence readiness/i)
      assert.match(overviewHtml, /Second-target policy/i)
      assert.match(overviewHtml, /Milestone 2 closeout discipline/i)
      assert.match(overviewHtml, /Toward C bounded runway/i)
      assert.match(overviewHtml, /pnpm milestone:rehearse:debian12-incus/)
      assert.match(overviewHtml, /pnpm milestone:preview:live-packet/)
      assert.match(overviewHtml, /capture_required/i)
      assert.match(overviewHtml, /SQLite stays active/i)
      ...
      assert.match(consoleHtml, /Deployment boundary/i)
      assert.match(consoleHtml, /Second-target policy/i)
      assert.match(consoleHtml, /Selected diagnostic detail/i)
      assert.match(consoleHtml, /Persistence readiness/i)
      assert.match(consoleHtml, /SQLite stays active/i)
